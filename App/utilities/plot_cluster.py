...
def run_cluster_analysis():
    ...
    print(f"{'Cluster':<12} | {'M_DM (M_sun)':<11} | {'M_tot (M_sun)':<11} | {'M_bar (M_sun)':<11} | {'r_s (kpc)':<9} | {'rho_s (M/kpc3)':<14} | {'% DM'}")
    print("-" * 92)
    ...
    for file in sorted(all_files):
        ...
        try:
            ...
            print(f"{cluster_name:<12} | {M_DM:<11.2e} | {M_tot_final:<11.2e} | {M_bar_tot:<11.2e} | {r_s:<9.1f} | {rho_s:<14.2e} | {perc_DM:.1f}%")

          
            f = 1.0
            M_tot_model = M_bar_tot + f * M200
            R_cluster = r200_from_M200(M_tot_model, rho_crit)
            v_center_tot = np.sqrt(np.maximum(0.0, G_grav * M_tot_model / R_cluster))
            ...
            sigma_bar_local = np.sqrt(np.maximum(0.0, G_grav * M_baryonic_r / (3.0 * r_sorted)))

          
            sigma_tot_local = np.sqrt(np.maximum(0.0, G_grav * M_tot_r / (3.0 * r_sorted)))
            sigma_tot = np.sqrt(np.maximum(0.0, G_grav * M_tot_model / (3.0 * r_sorted)))
            sigma_bar = np.sqrt(np.maximum(0.0, G_grav * M_bar_tot / (3.0 * r_sorted)))
            v_bar = rng.normal(loc=sigma_bar, scale= sigma_global, size=len(r_sorted))
            v_dm  = rng.normal(loc=sigma_tot, scale=sigma_global, size=len(r_sorted))
