...
def safe_click(*actions):
    """Esegue in sequenza una o più azioni quando si clicca un pulsante, evitando problemi di serializzazione."""
    def _handler():
        for act in actions:
            try:
                if callable(act):
...
def accessible_notify(text: str, type_: str = "info"):
    """Mostra un messaggio accessibile e visivamente temporaneo"""
...
""")
    ...
    spectra_csv_dir = "/workspaces/Cosmo-Edu_Lab/App/galaxy_spectra/csv_converted"
    fits_parent_dir = "/workspaces/Cosmo-Edu_Lab/App/galaxy_fits"  # used to optionally compare with original FITS headers (if present)
    sn_file = "/workspaces/Cosmo-Edu_Lab/App/data/supernovae.txt"

    # Ensure directories exist
    if not os.path.isdir(spectra_csv_dir):
        # fallback: look in parent dir for csv files
        spectra_csv_dir = os.path.join(os.path.dirname(__file__), "galaxy_spectra", "csv_converted")
    if not os.path.isdir(spectra_csv_dir):
    ...
    with ui.tab_panels(tabs, value=one):
        with ui.tab_panel(one):
            title("Activity 1 — Cosmological Redshift (SDSS spectra)")

            ui.markdown("""
**Goal:** show that galaxy spectral lines are shifted to longer wavelengths (redshift), measure *z* by matching observed peaks to known rest-frame atomic lines, compute velocities (non-relativistic and relativistic), and compare them over *z*.
""")

            # Known rest-frame spectral lines (Å)
            known_lines = {
                "Hα": 6562.8,
                "Hβ": 4861.3,
                "[O III]": 5006.8,
            ...
            file_select = ui.select(csv_files, label="Choose spectrum CSV (wavelength_A, flux)").classes("w-96")

            # interactive parameters
            peak_height = ui.number(label="Peak height threshold (relative 0-1)", value=0.4, format="%.3f").classes("w-64")
            peak_distance = ui.number(label="Min distance between peaks (pixels)", value=10, format="%.0f").classes("w-64")

            # containers for plots and results
            activity1_plots = ui.column().classes("w-full")
            results_box = ui.column().classes("w-full")

            def try_read_original_fits_redshift(csv_filename):
                """
                Optional: try to open a corresponding FITS in fits_parent_dir to find header redshift for comparison.
                We'll look for files that share the base name but .fits extension and try common header keys.
                """
                try:
                    base = os.path.splitext(csv_filename)[0]
                    candidate = None
                    for ext in (".fits", ".fit", ".FITS"):
                        cand_path = os.path.join(fits_parent_dir, base + ext)
                        if os.path.exists(cand_path):
                            candidate = cand_path
                            break
                    if candidate is None:
                        # try to find any FITS with matching plate-mjd-fiber pattern inside name
                        for f in os.listdir(fits_parent_dir):
                            if base in f and f.lower().endswith(".fits"):
                                candidate = os.path.join(fits_parent_dir, f)
                                break
                    if candidate is None:
                        return None, None
                    # lazy import astropy here
                    try:
                        from astropy.io import fits as _fits
                    except Exception:
                        return candidate, None
                    with _fits.open(candidate) as hdul:
                        hdr = hdul[0].header
                        # common redshift keys
                        for k in ("Z", "REDSHIFT", "Z_VAC", "Z_HELIO", "Z_NOQSO"):
                            if k in hdr:
                                return candidate, hdr.get(k)
                    return candidate, None
                except Exception:
                    return None, None

            def analyze_spectrum():
                activity1_plots.clear()
                results_box.clear()

                if not file_select.value:
                    accessible_notify("Please select a spectrum CSV file.", type_="warning")
                    return

                csv_path = os.path.join(spectra_csv_dir, file_select.value)
                try:
                    df = pd.read_csv(csv_path)
                except Exception as e:
                    accessible_notify(f"Could not read CSV: {e}", type_="error")
                    return

                if ("wavelength_A" not in df.columns) or ("flux" not in df.columns):
                    accessible_notify("CSV must contain 'wavelength_A' and 'flux' columns.", type_="error")
                    return
                ...
                # basic cleaning
                finite_mask = np.isfinite(wl) & np.isfinite(flux)
                wl = wl[finite_mask]
                flux = flux[finite_mask]

                # smoothing optional (small Savitzky-Golay could be added — here we keep raw)
                # Normalize flux to [0,1] for peak finding
                if flux.max() - flux.min() == 0:
                    fnorm = flux
                else:
                    fnorm = (flux - np.nanmin(flux)) / (np.nanmax(flux) - np.nanmin(flux))

                # find peaks
                try:
                    ph = float(peak_height.value)
                    pdist = int(max(1, peak_distance.value))
                except Exception:
                    ph, pdist = 0.4, 10

                peaks_idx, props = find_peaks(fnorm, height=ph, distance=pdist)
                peak_wls = wl[peaks_idx]
                peak_heights = props["peak_heights"] if "peak_heights" in props else fnorm[peaks_idx]

                # For each known line, find the closest detected peak (if any) and compute z
                measured = []
                for line_name, lambda0 in known_lines.items():
                    if len(peak_wls) == 0:
                        continue
                    idx = np.argmin(np.abs(peak_wls - lambda0))
                    lambda_obs = peak_wls[idx]
                    dz = (lambda_obs - lambda0) / lambda0
                    measured.append({
                        "line": line_name,
                ...
                # compute an average z from measured matched lines (robust: median)
                if len(measured) > 0:
                    zs = np.array([m["z"] for m in measured])
                    z_med = float(np.median(zs))
                else:
                    z_med = np.nan

                c_km_s = 299792.458  # km/s
                v_nr = c_km_s * z_med if np.isfinite(z_med) else np.nan
                v_rel = c_km_s * ( (1+z_med)**2 - 1 ) / ( (1+z_med)**2 + 1 ) if np.isfinite(z_med) else np.nan

                # Try to read original FITS header redshift (optional)
                fits_path, header_z = try_read_original_fits_redshift(file_select.value)

                # Plot: flux vs wavelength with peaks and annotated matched lines
                with activity1_plots:
                    with ui.pyplot(figsize=(9, 4)):
                        plt.plot(wl, flux, lw=1, label="flux")
                        plt.scatter(peak_wls, flux[peaks_idx], color="red", s=30, label="detected peaks")
                        for m in measured:
                            plt.axvline(m["lambda_obs"], ls="--", alpha=0.6)
                            plt.text(m["lambda_obs"], np.nanmax(flux)*0.95, f"{m['line']} (z={m['z']:.4f})",
                ...
                # Plot: v_nr and v_rel vs z (single point for this spectrum, but we show as marker + annotation)
                with activity1_plots:
                    with ui.pyplot(figsize=(6,4)):
                        zs_plot = np.linspace(0, max(0.5, (z_med*1.5 if np.isfinite(z_med) else 0.5)), 300)
                        v_nr_curve = c_km_s * zs_plot
                        v_rel_curve = c_km_s * ( (1+zs_plot)**2 - 1 ) / ( (1+zs_plot)**2 + 1 )
                        plt.plot(zs_plot, v_nr_curve, label="v (non-relativistic) = c·z", lw=1)
                        plt.plot(zs_plot, v_rel_curve, label="v (relativistic)", lw=1)
                        if np.isfinite(z_med):
                            plt.scatter([z_med], [v_nr], color="C0", s=50, label=f"this spectrum (v_nr={v_nr:.1f} km/s)")
                            plt.scatter([z_med], [v_rel], color="C1", s=50, label=f"this spectrum (v_rel={v_rel:.1f} km/s)")
                        plt.xlabel("Redshift z")
                        plt.ylabel("Velocity [km/s]")
                        plt.title("Non-relativistic vs Relativistic velocity")
                        plt.legend()
                        plt.grid(alpha=0.3)
                        plt.tight_layout()

                # Results box: table and numbers
                with results_box:
                    ui.markdown("### Matched lines and computed redshifts")
                    if len(measured) > 0:
                        rows = []
                        for m in measured:
                            rows.append({
                                "line": m["line"],
                                "lambda0": f"{m['lambda0']:.1f}",
                                "lambda_obs": f"{m['lambda_obs']:.2f}",
                                "z": f"{m['z']:.5f}"
                            })
                        aria_table(
                            columns=[
                            ...
                            rows=rows,
                            label="Matched spectral lines"
                        )
                    else:
                        ui.label("No matched lines found with current peak parameters. Try lowering the peak threshold.").classes("text-yellow-700")

                    ui.markdown("### Summary")
                    ui.label(f"Median z from matched lines: {z_med:.5f}").classes("mt-1")
                    ui.label(f"Non-relativistic velocity v = c·z: {v_nr:.1f} km/s").classes("mt-1")
                    ui.label(f"Relativistic velocity v_rel: {v_rel:.1f} km/s").classes("mt-1")
                    if fits_path:
                        ui.label(f"Found original FITS: {fits_path}").classes("text-sm")
                        if header_z:
                            ui.label(f"Header redshift (from FITS header): {header_z}").classes("text-sm")
                        else:
                            ui.label("No redshift key found in FITS header.").classes("text-sm")
                    ui.markdown("### Formulae used (for reference)")
                    ui.markdown(
                        r"""
- Redshift: \( z = \frac{\lambda_{\rm obs} - \lambda_0}{\lambda_0} \)
- Non-relativistic velocity: \( v_{\rm nr} = c\, z \)
- Relativistic velocity: \( v_{\rm rel} = c\,\frac{(1+z)^2 - 1}{(1+z)^2 + 1} \)
"""
                    )

            aria_button("Analyze spectrum", "Compute redshift and show plots", on_click=analyze_spectrum).classes("mt-3")

    # ---------- Activity 2: Hubble Law ----------
    with ui.tab_panels(tabs, value=two):
        with ui.tab_panel(two):
            title("Activity 2 — Hubble's Law (Pantheon supernovae)")

            ui.markdown("""
**Goal:** compute velocities, distance moduli, luminosity distances, estimate H₀ via linear fit for low-z SNe, compare non-relativistic and relativistic velocities, show residuals and Hubble diagram, and compare to ΛCDM model (Ω_m=0.3, Ω_Λ=0.7, flat).
""")

            # Inputs and options
            h0_guess = ui.number(label="Initial guess for H₀ [km/s/Mpc]", value=70.0, format="%.2f").classes("w-64")
            M_B_input = ui.number(label="Absolute magnitude M_B (SNe Ia)", value=-19.3, format="%.3f").classes("w-64")
            z_cut_low = ui.number(label="Low-z cutoff for linear fit (z < ...)", value=0.1, format="%.3f").classes("w-64")
            cosmology_box = ui.column().classes("w-full")

            plots2 = ui.column().classes("w-full")

            def hubble_analysis():
                plots2.clear()
                if not os.path.exists(sn_file):
                    accessible_notify(f"Supernovae file not found: {sn_file}", type_="error")
                    return

                # Try reading relevant columns — the user specified the table column names earlier
                try:
                    # We'll attempt to read whitespace-delimited table; if fails, try CSV
                    df_sn = pd.read_csv(sn_file, delim_whitespace=True, comment="#", engine="python")
                except Exception:
                    try:
                        df_sn = pd.read_csv(sn_file)
                    except Exception as e:
                        accessible_notify(f"Could not read supernovae file: {e}", type_="error")
                        return

                # Required columns: zcmb (or z), dz (or dzcmb), mb (or mag_b), dmb (or err_mag)
                # Try to find matching columns
                col_map = {}
                for col in df_sn.columns:
                    cl = col.lower()
                    if cl in ("zcmb","z_cmb","z_cmb:"):
                        col_map["zcmb"] = col
                    if cl in ("z","zcmb"):
                        col_map.setdefault("zcmb", col)
                    if "dz" in cl and "zcmb" in cl:
                        col_map.setdefault("dz","dz")  # best effort
                    if "mb" == cl or cl.startswith("mag") or "m_b" in cl or cl=="mb":
                        col_map.setdefault("mb", col)
                    if "dmb" in cl or "err" in cl or "e_mag" in cl:
                        col_map.setdefault("dmb", col)

                # require columns explicitly if possible, otherwise attempt known names:
                needed = {}
                for k in ("zcmb","mb"):
                    if k in col_map:
                        needed[k] = col_map[k]
                        ...
                        for tryname in ("zcmb","z","Zcmb","Z_CMB","zCMB","zcmb"):
                            if tryname in df_sn.columns:
                                needed["zcmb"] = tryname
                                break
                # try defaults
                if "zcmb" not in needed:
                    if "z" in df_sn.columns:
                        needed["zcmb"] = "z"
                if "mb" not in needed:
                    for tryname in ("mb","mag_b","magb","Bmag","m_b"):
                        if tryname in df_sn.columns:
                            needed["mb"] = tryname
                            break

                if "zcmb" not in needed or "mb" not in needed:
                    accessible_notify("Could not find required columns 'zcmb' and 'mb' in supernovae file. Please check file format.", type_="error")
                    return

                # dz and dmb optional
                dz_col = None
                dmb_col = None
                for tryname in ("dz","dzcmb","zerr","z_err"):
                    if tryname in df_sn.columns:
                        dz_col = tryname
                        break
                for tryname in ("dmb","err_mb","errmag","e_mag","sig_mb"):
                    if tryname in df_sn.columns:
                        dmb_col = tryname
                        break
                ...
                df = pd.DataFrame()
                df["zcmb"] = pd.to_numeric(df_sn[needed["zcmb"]], errors="coerce")
                df["mb"] = pd.to_numeric(df_sn[needed["mb"]], errors="coerce")
                if dz_col:
                    df["dz"] = pd.to_numeric(df_sn[dz_col], errors="coerce")
                else:
                    df["dz"] = np.nan
                if dmb_col:
                    df["dmb"] = pd.to_numeric(df_sn[dmb_col], errors="coerce")
                else:
                    df["dmb"] = np.nan

                df = df.dropna(subset=["zcmb","mb"])

                # Constants
                c_km_s = 299792.458  # km/s
                M_B = float(M_B_input.value)

                # compute velocities and distances
                df["v_nr"] = c_km_s * df["zcmb"]
                df["v_rel"] = c_km_s * ( (1+df["zcmb"])**2 - 1 ) / ( (1+df["zcmb"])**2 + 1 )
                df["mu"] = df["mb"] - M_B
                df["d_L_pc"] = 10 ** ((df["mu"] + 5) / 5)
                df["d_L_Mpc"] = df["d_L_pc"] / 1e6

                # Filter low-z for linear H0 fit
                zcut = float(z_cut_low.value)
                lowz = df[df["zcmb"] < zcut].dropna(subset=["d_L_Mpc","v_nr"])

                # Estimate H0 via provided formula H0 = sum(v_i dL_i) / sum(dL_i^2)
                if len(lowz) > 1:
                    num = (lowz["v_nr"] * lowz["d_L_Mpc"]).sum()
                    den = (lowz["d_L_Mpc"]**2).sum()
                    H0_est_formula = num / den
                    # also do linear regression v vs d_L
                    slope, intercept, r_value, p_value, std_err = linregress(lowz["d_L_Mpc"], lowz["v_nr"])
                    H0_est_reg = slope
                else:
                    H0_est_formula = np.nan
                    slope, intercept, r_value, p_value, std_err = (np.nan, np.nan, np.nan, np.nan, np.nan)
                    H0_est_reg = np.nan

                # Residuals
                df["residual_nr"] = df["v_nr"] - H0_est_reg * df["d_L_Mpc"]
                df["residual_rel"] = df["v_rel"] - H0_est_reg * df["d_L_Mpc"]

                # Percent difference between v_nr and v_rel
                df["pct_diff_v"] = 100.0 * (df["v_nr"] - df["v_rel"]) / df["v_rel"]
                # limit pct where v_rel zero
                df.loc[np.isfinite(df["v_rel"])==False, "pct_diff_v"] = np.nan

                # ---------------------------
                # Plots
                # ---------------------------
                with plots2:
                    # 1) v vs d_L with fit (Hubble diagram linear version)
                    with ui.pyplot(figsize=(7,4)):
                        plt.scatter(df["d_L_Mpc"], df["v_nr"], s=20, alpha=0.6, label="v = c·z (non-rel.)")
                        if len(lowz) > 1 and np.isfinite(slope):
                            xfit = np.linspace(0, max(lowz["d_L_Mpc"].max(), df["d_L_Mpc"].quantile(0.95)), 200)
                            plt.plot(xfit, intercept + slope*xfit, "r--", lw=2, label=f"Linear fit (z<{zcut}): H₀={H0_est_reg:.1f} km/s/Mpc")
                        plt.xlabel("Luminosity distance d_L [Mpc]")
                        plt.ylabel("Velocity v [km/s]")
                        plt.title("Hubble Law: v vs d_L (non-relativistic)")
                        plt.grid(alpha=0.3)
                        plt.legend()
                        plt.tight_layout()

                    # 2) magnitude vs z with Hubble straight model & LCDM curve
                    with ui.pyplot(figsize=(7,4)):
                        plt.errorbar(df["zcmb"], df["mb"], yerr=df.get("dmb", None), fmt="o", ms=4, alpha=0.5, label="SNe (data)")
                        # Hubble-model magnitude using H0_est_reg (if available) with d_L = c z / H0
                        if np.isfinite(H0_est_reg):
                            z_model = np.linspace(0.001, max(1.0, df["zcmb"].max()), 300)
                            dL_hubble_pc = (c_km_s * z_model / H0_est_reg) * 1e6  # km/s cancels -> convert to pc
                            mu_hubble = 5.0 * np.log10(dL_hubble_pc) - 5.0
                            mb_hubble = M_B + mu_hubble
                            plt.plot(z_model, mb_hubble, "r--", label=f"Hubble model (H₀={H0_est_reg:.1f})")

                        # LCDM model: Omega_m=0.3, Omega_L=0.7, flat
                        Omega_m = 0.3
                        Omega_k = 0.0
                        Omega_L = 0.7
                        H0_model = float(h0_guess.value)

                        def E(z):
                            return math.sqrt(Omega_m * (1+z)**3 + Omega_k * (1+z)**2 + Omega_L)

                        # compute dL for LCDM via integral
                        def dL_LCDM(z):
                            # returns d_L in parsec
                            # integral I = ∫_0^z dz'/E(z')
                            I, _ = quad(lambda zp: 1.0 / math.sqrt(Omega_m*(1+zp)**3 + Omega_k*(1+zp)**2 + Omega_L), 0.0, z)
                            # c/H0 in km/s divided by (km/s/Mpc) -> Mpc; convert to pc
                            dL_Mpc = (c_km_s / H0_model) * (1+z) * I
                            return dL_Mpc * 1e6

                        # compute LCDM curve (this can be somewhat slow for many points; keep moderate resolution)
                        z_lcdm = np.linspace(0.001, max(1.0, df["zcmb"].max()), 150)
                        dL_lcdm_pc = np.array([dL_LCDM(zp) for zp in z_lcdm])
                        mu_lcdm = 5.0 * np.log10(dL_lcdm_pc) - 5.0
                        mb_lcdm = M_B + mu_lcdm
                        plt.plot(z_lcdm, mb_lcdm, "g-", label=f"ΛCDM (Ωm={Omega_m}, ΩΛ={Omega_L})")

                        plt.xlabel("Redshift z")
                        plt.ylabel("Apparent magnitude m_B")
                        plt.title("Hubble diagram: m_B vs z (Data, Hubble model, ΛCDM)")
                        plt.gca().invert_yaxis()
                        plt.legend()
                        plt.grid(alpha=0.3)
                        plt.tight_layout()

                    # 3) Velocity non-rel vs relativistic vs z (many points)
                    with ui.pyplot(figsize=(7,4)):
                        zvals = np.linspace(0.0, max(df["zcmb"].max(), 0.5), 400)
                        v_nr_curve = c_km_s * zvals
                        v_rel_curve = c_km_s * ( (1+zvals)**2 - 1 ) / ( (1+zvals)**2 + 1 )
                        plt.plot(zvals, v_nr_curve, label="v_nr = c·z")
                        plt.plot(zvals, v_rel_curve, label="v_rel (relativistic)")
                        plt.xlabel("Redshift z")
                        plt.ylabel("Velocity [km/s]")
                        plt.title("Non-relativistic vs Relativistic velocity as function of z")
                        plt.legend()
                        plt.grid(alpha=0.3)
                        plt.tight_layout()

                # Info & numeric results
                with plots2:
                    plot_info_box({
                        "Estimated H₀ (formula)": f"{H0_est_formula:.2f} km/s/Mpc" if np.isfinite(H0_est_formula) else "n/a",
                        "Estimated H₀ (regression)": f"{H0_est_reg:.2f} km/s/Mpc" if np.isfinite(H0_est_reg) else "n/a",
                        "Points (all)": str(len(df)),
                        f"Points (z<{zcut})": str(len(lowz)),
                        "Omega_m": f"{Omega_m}",
                        "Omega_Lambda": f"{Omega_L}"
                    }, title="Fit & Cosmology Summary")

                # Show residual histogram and percent diff summary
                with plots2:
                    with ui.pyplot(figsize=(6,3)):
                        plt.hist(df["pct_diff_v"].dropna(), bins=60, alpha=0.7)
                        plt.xlabel("Percent difference (v_nr - v_rel)/v_rel * 100%")
                        plt.title("Percent difference between non-relativistic and relativistic velocities")
                        plt.grid(alpha=0.3)
                        plt.tight_layout()

                    # table of a few sample SNe with computed values
                    sample_rows = []
                    for _, row in df.head(20).iterrows():
                        sample_rows.append({
                            "z": f"{row['zcmb']:.5f}",
                            "v_nr_km_s": f"{row['v_nr']:.1f}",
                            "v_rel_km_s": f"{row['v_rel']:.1f}",
                            "d_L_Mpc": f"{row['d_L_Mpc']:.3f}",
                            "m_B": f"{row['mb']:.3f}"
                        })
                    aria_table(
                        columns=[
                            {"name":"z","label":"z","field":"z"},
                            {"name":"v_nr_km_s","label":"v_nr [km/s]","field":"v_nr_km_s"},
                            {"name":"v_rel_km_s","label":"v_rel [km/s]","field":"v_rel_km_s"},
                            {"name":"d_L_Mpc","label":"d_L [Mpc]","field":"d_L_Mpc"},
                            {"name":"m_B","label":"m_B","field":"m_B"},
                        ],
                        rows=sample_rows,
                        label="Sample supernovae computed values"
                    )

            aria_button("Run Hubble analysis", "Compute Hubble diagrams and fits", on_click=hubble_analysis).classes("mt-3")

            # formulas & explanation box
            with cosmology_box:
                ui.markdown("### Formulae & Remarks (reference)")
                ui.markdown(r"""
- Velocity (non-relativistic): \( v = c z \).
- Distance modulus: \( \mu = m_B - M_B = 5 \log_{10}(d_L/{\rm pc}) - 5 \).
- Luminosity distance: \( d_L = 10^{(\mu+5)/5} \, {\rm pc} \) (divide by \(10^6\) to get Mpc).
- Hubble law (low z linear): \( v = H_0 d_L \). Estimate \( H_0 \approx \frac{\sum v_i d_{L,i}}{\sum d_{L,i}^2} \) for low-z SNe.
- Relativistic velocity: \( v = c \frac{(1+z)^2 - 1}{(1+z)^2 + 1} \).
- ΛCDM luminosity distance (flat): \( d_L(z) = \frac{c(1+z)}{H_0} \int_0^z \frac{dz'}{E(z')} \), with \( E(z) = \sqrt{\Omega_m (1+z)^3 + \Omega_k (1+z)^2 + \Omega_\Lambda} \).
""")

    # Add standard module UI (AI tutor and reflections)
    standard_module_ui("Module 3 — The Expanding Universe")
...
""")
    ...
    with ui.tabs().classes('w-full justify-center') as tabs:
        ...
        four = ui.tab('Photons decoupling', icon='query_stats').props('aria-label="Activity 4: Photons decoupling"')
        five = ui.tab('Entropy', icon='insights').props('aria-label="Activity 5: Entropy"')
            ...
            with container:
                ...
                with ui.row().classes("w-full items-start justify-between gap-8 mt-4").style("flex-wrap: nowrap; align-items: flex-start;"):
                    ...
                    with ui.column().classes("w-1/2 min-w-[480px] max-w-[640px] flex-shrink-0"):
                        ...
                        info_box("Wien’s Law Calculator: T=lambda_max/b"
                      
                        "\nInsert the observed peak wavelength 'lambda_max' to compute the temperature")
...
""", sanitize=False).props("id=planck-info-content role=document aria-live=polite")
                ...
                with ui.row().classes("w-full items-start justify-between gap-8 mt-4"):
                    ...
                    "Dataset: Redshift (z), CMB Temperature (K), T/z+1, T_error"
                        ...
                        with ui.dialog().props(
                            ...
                            ).classes("prose").props("id=info-content role=document aria-live=polite")
    ...
    'aria-modal="true" role="dialog" aria-labelledby="legend-title" aria-describedby="legend-content"'
                            ...
                            ).classes("prose").props("id=legend-content role=document aria-live=polite")
                    ...
                    with ui.card().classes("p-6 w-full max-w-2xl min-w-[600px] bg-gray-900 text-white rounded-xl shadow-xl"):
                        ui.markdown("### 🧮 CMB Thermodynamics Exercise").classes("text-2xl font-bold mb-4 text-blue-300")
                        ui.markdown("Fill in the missing variables to connect thermodynamics and cosmology:").classes("text-sm text-gray-300")
            ...
            with container:
                ...
                with ui.row().classes("w-full items-start justify-between gap-8 mt-4"):
                    ...
                    with ui.card().classes("p-6 w-full max-w-2xl bg-gray-900 text-white rounded-xl shadow-xl"):
                        ui.markdown("### 🧮 Radiation & Matter Density Evolution").classes("text-2xl font-bold mb-4 text-blue-300")
                    ...
                    with ui.column().classes("w-1/2 items-center bg-gray-900 p-4 rounded-xl shadow-lg"):
                        ...
                        ui.label("Energy density evolution").classes("text-lg font-semibold text-blue-300 mb-2")
                        ...
                        def plot_curves(show_curves=False):
                            ...
                            with ui.row().classes("gap-4 mt-4"):
                                z_eq_input = ui.number(label="z_eq", value=None).props("dense filled")
                                T_eq_input = ui.number(label="T_eq [K]", value=None).props("dense filled")
                ...
                with ui.dialog().props('aria-modal="true" role="dialog" aria-labelledby="info-title" aria-describedby="info-content"') as info_dlg, ui.card():
                    ...
                    ).classes("prose text-gray-200").props("id=info-content")
...
"- **ν** = photon frequency [Hz] \n"
                    ...
                    ).classes("prose text-gray-200").props("id=legend-content")
    ...
    with ui.tab_panels(tabs, value=one).classes('w-full p-4'):
        ...
        with ui.tab_panel(four).props('role=tabpanel aria-label="Photons decoupling"'):
            ui.label("Activity 3: Photons decoupling").classes('text-lg text-gray-600')

      
        with ui.tab_panel(five).props('role=tabpanel aria-label="Entropy"'):
            ui.label("Activity 4: Entropy").classes('text-lg text-gray-600')
...
''', sanitize=False)
    ...
    'id=audio_status role=status aria-live=polite aria-atomic=true tabindex=0'
                ...
                alpha_slider = aria_slider(min=0.0, max=2.0, value=0.0, step=0.01,  aria_label="Dark matter fraction control slider").props('aria-describedby=alpha_slider_label label-always')
                ...
                try:
                    ...
                    warning_box("Error: File not found.").classes('text-red-500')
                    DATA_LOADED = False
...
$$
    ...
    'role=status aria-live=polite tabindex=0 aria-label=Plot showing chi-square minimization curve; X axis is dark matter mass in solar masses, Y axis is chi-square per degree of freedom'
                ...
                with ui.row().classes('w-full gap-4 justify-start'):
                    with ui.column().classes('flex-1 items-center '):
                        aria_image(os.path.join(GALAXY_IMG_PATH, "NGC3198.jpg"),"Image of the NGC3198 galaxy").classes("w-full max-w-xl")
...
""").classes('w-full text-center text-base italic mt-2')
                    ...
                    with ui.column().classes('flex-1 items-center '):
                        ...
                        if os.path.exists(table_filepath):
                            ...
                            ui.table.from_pandas(df).props('aria-label=Table of the galaxy characteristics role=table').classes('w-full max-w-xl')
                            reference_box("""
...
''', sanitize=False)
    ...
    "DM factor": f"{f:.2f}"
                    ...
                    with ui.row().classes(' w-full gap-4 justify-center'):
                        ...
                        with ui.column().classes('flex-1 items-center'):
                            aria_image(os.path.join(CLUSTER_IMG_PATH, "coma_img.jpg"), "Image of the Coma cluster").classes('w-full max-w-xl')
                            reference_box("""
**Image reference:**  
- [ESA Hubble](https://esahubble.org/)
""").classes('w-full text-center text-base italic mt-2')
                        ...
                        with ui.column().classes('flex-1 items-center'):
                            ...
                            if os.path.exists(table_filepath):
                                df_table = pd.read_csv(table_filepath)
                                ui.table.from_pandas(df_table).props('aria-label=Table of Coma cluster characteristics role=table').classes('w-full max-w-xl')
                                reference_box("""
