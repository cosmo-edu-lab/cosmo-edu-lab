...
def create_page():
    ...
    """)
                    ...
                    with ui.dialog() as chi2_info_dialog, ui.card().classes('p-4 w-full max-w-[1200px] overflow-x-auto'):
                        html_info_box(r"""
        <h3>Parabolic interpolation and compute chi^2 minimization</h3>
        <p>Use the formula below to compute the χ² minimum of a parabola defined by three points (a, f(a)), (b, f(b)), (c, f(c)).</p>
        

        <ul>
            <li><b>1a)</b> Choose 3 values of dark matter fraction (f) with the slider above to compute the χ² at that point.</li>
            <li><b>1b)</b> Press <b>'Add χ² point'</b> button to add the point to the χ² plot.</li>
            <li><b>2a)</b> Input 3 values for dark matter mass points (x-coordinates).</li>
            <li><b>2b)</b> Press <b>'compute χ² and minimum'</b> button to calculate the χ² at those points and check the minimum on the plot.</li>
        </ul>

        <div style="margin-top: 15px; background: rgba(255,255,255,0.6); padding: 15px; border: 1px solid #0284c7; border-radius: 8px; color:#111;">
            <b>Formula:</b><br>
            $$
...
<button id="audio-button" role="button" aria-label=" or deactivate audio" aria-pressed="false" onclick="initAudio()" 
        ...
        'role=status aria-live=polite tabindex=0 aria-label=Plot showing chi-square minimization curve; X axis is dark matter mass in solar masses, Y axis is chi-square per degree of freedom'
                    ...
                    with ui.row().classes('w-full no-wrap items-start justify-center gap-x-8'):
                        ...
                        morph_plot_container = ui.column().classes("w-full md:w-[30%] min-w-[300px]")
            ...
            f"$v_{{obs}} = {v_obs_ngc.max():.1f} \\, \mathrm{{km/s}}$\n"
            f"$v_{{sim}} = {v_total_curve.max():.1f} \\, \mathrm{{km/s}}$"
        )
        ...
        'role=status aria-live=polite tabindex=0 aria-label=Total mass of the galaxy (from data), DMyonic mass (computed from data) and simulated mass updated with new dark matter value'
                        ...
                        def update_morphology_plot():
                                ...
                                with morph_plot_container:
                                    ...
                                    plt.close()
                                    with ui.pyplot(figsize=(4,3), close=False,clear=True):
                                     ...
                                     fill=False, linestyle='--', alpha=0.5, zorder=1)
                                        ...
                                        galaxy_name_for_title = current_galaxy_name.removesuffix('.txt')
                                        ax.set_title(f"Galaxy structure (top-view): {galaxy_name_for_title}",fontsize=10,fontweight='bold')
        ...
        'role=status aria-live=polite tabindex=0 aria-label=Galaxy morphology representation (from above), showing disk with baryonic component (stars,gas) and halo appears adding dark matter (slider)'
                        ...
                        def update_morphology_plot_3d():
                            ...
                            # Raggi stimati dai dati
                            R_bulge = np.average(r_ngc, weights=np.maximum(0, v_bul_ngc**2)) if np.any(v_bul_ngc > 0) else 0.5
                            R_disk  = np.average(r_ngc, weights=np.maximum(0, v_disk_ngc**2)) if np.any(v_disk_ngc > 0) else 5
                            R_halo  = np.sqrt(M_dm_tot / (M_vis_tot+1e-12)) * np.max(r_ngc) if M_dm_tot > 0 else 0

                            # --- FORZATURA VISIVA DIDATTICA ---
                            # Per mostrare bene la morfologia, assicuriamo che le proporzioni siano visibili
                            # Regola: Il disco deve essere più largo del Bulge, e l'Alone più largo del Disco.
                            if R_disk < R_bulge * 2.0:
                                R_disk_vis = R_bulge * 3.0 
                            else:
                                R_disk_vis = R_disk

                            #with morph_plot_container3d:
                            #    morph_plot_container3d.clear()
                                plt.close('all') 
                                
                                with ui.pyplot(figsize=(10, 10), close=False, clear=True) as plot_element:
                                    fig = plot_element.fig 
                                    ax = fig.add_subplot(111, projection='3d')
                                    
                                    # Limite grafico: Include l'alone se c'è, altrimenti il disco
                                    if f > 0 and R_halo > R_disk_vis:
                                        limit_R = min(R_halo, R_disk_vis * 2.5) # Non fare zoom out eccessivo se l'alone è infinito
                                    else:
                                        limit_R = R_disk_vis * 1.2
                                    
                                    maxR = limit_R

                                    # --- 1. BULGE (Sfera Solida Centrale) ---
                                    # Rappresenta il nucleo denso sferico
                                    u = np.linspace(0, 2 * np.pi, 25)
                                    v = np.linspace(0, np.pi, 25)
                                    x_b = R_bulge * np.outer(np.cos(u), np.sin(v))
                                    y_b = R_bulge * np.outer(np.sin(u), np.sin(v))
                                    z_b = R_bulge * np.outer(np.ones(np.size(u)), np.cos(v))
                                    
                                    ax.plot_surface(x_b, y_b, z_b, color='#FFD700', alpha=1.0, zorder=10, linewidth=0, shade=False)

                                    # --- 2. DISCO (Anelli Piatti) ---
                                    # Rappresenta la rotazione sul piano galattico. Z rimane vicino a 0.
                                    theta = np.linspace(0, 2 * np.pi, 80)
                                    radii = np.linspace(R_bulge * 1.1, R_disk_vis, 10) 
                                    
                                    for i, r in enumerate(radii):
                                        x_ring = r * np.cos(theta)
                                        y_ring = r * np.sin(theta)
                                        
                                        # TRUCCO VISIVO: Aggiungiamo un minimo "jitter" su Z per dare volume, 
                                        # ma mantenendolo piatto. Spessore = 2% del raggio.
                                        z_jitter = (np.random.rand(len(theta)) - 0.5) * (R_disk_vis * 0.02)
                                        z_ring = np.zeros_like(x_ring) + z_jitter

                                        # Usiamo alpha diverso per dare effetto sfumato verso l'esterno
                                        alpha_ring = 0.8 - (i / len(radii)) * 0.5 
                                        ax.plot(x_ring, y_ring, z_ring, color='crimson', lw=1.5, alpha=alpha_ring, zorder=5)

                                    # Bordo esterno del disco
                                    x_edge = R_disk_vis * np.cos(theta)
                                    y_edge = R_disk_vis * np.sin(theta)
                                    ax.plot(x_edge, y_edge, np.zeros_like(x_edge), color='crimson', lw=2, linestyle='--', label='Disk Extent')

                                    # --- 3. BLACK HOLE (Centro esatto) ---
                                    ax.scatter([0], [0], [0], color='black', s=200, marker='X', 
                                            edgecolors='white', linewidth=1.5, zorder=100, label='Black Hole')

                                    # --- 4. ALONE DM (Sfera Grande Trasparente) ---
                                    if f > 0 and R_halo > 0:
                                        # Disegniamo l'alone un po' più grande del disco per mostrare che lo contiene
                                        R_h_draw = max(R_disk_vis * 1.2, min(R_halo, maxR))
                                        
                                        u = np.linspace(0, 2 * np.pi, 14)
                                        v = np.linspace(0, np.pi, 10)
                                        x_h = R_h_draw * np.outer(np.cos(u), np.sin(v))
                                        y_h = R_h_draw * np.outer(np.sin(u), np.sin(v))
                                        z_h = R_h_draw * np.outer(np.ones(np.size(u)), np.cos(v))
                                        
                                        # Wireframe verde lime per far vedere l'interno
                                        ax.plot_wireframe(x_h, y_h, z_h, color='limegreen', alpha=0.15, linewidth=0.8, zorder=1)
                                    ...
                                    # Angolo di vista: 30 gradi di elevazione è perfetto per vedere sia la piattezza del disco che il volume dell'alone
                                    ax.view_init(elev=30, azim=-60)
                                    
                                    galaxy_clean = current_galaxy_name.replace('.txt', '')
                                    plt.title(f"3D: {galaxy_clean} (Flat Disk inside Halo)", fontweight='bold', fontsize=10)
                                    
                                    # Legenda
                                    legend_elements = [
                                        mlines.Line2D([0], [0], marker='X', color='w', markerfacecolor='black', markersize=8, label='Black Hole'),
                                        mlines.Line2D([0], [0], marker='o', color='w', markerfacecolor='#FFD700', markersize=8, label='Bulge (Sphere)'),
                                        mlines.Line2D([0], [0], color='crimson', lw=2, label='Stellar Disk (Flat)'),
                                    ]
                                    if f > 0:
                                        legend_elements.append(mlines.Line2D([0], [0], color='limegreen', lw=1, linestyle='--', label='DM Halo (Sphere)'))

                                    plt.legend(handles=legend_elements, loc='upper left', fontsize=7, framealpha=0.9)
                                    plt.tight_layout()
                                    
                                    # Elemento descrittivo per screen readers
                                    ui.element('div').props(
                                        'role=status aria-label=3D plot showing a flat red disk representing stars inside a large spherical green wireframe representing dark matter halo'
                                    )
    ...
    """).props('role=dialog aria-modal=true aria-label="Mathematical explanation of the steps to compute baryonic velocity and mass"')
                    ...
                    with ui.dialog().props('role="dialog" aria-label="legend-title" aria-describedby="legend-desc"') as legend_dialog, ui.card().classes("p-4 w-full max-w-2xl overflow-x-auto"):
                        html_info_box(r"""
        <h3>Legend of Symbols and Units</h3>
        <ul>
            ...
            <li><b>G</b> = <span class="math">\( 4.30091 \times 10^{-6} \)</span> kpc·(km/s)²·M<span class="math">\(_\odot^{-1}\)</span></li>
            <li><b>M_baryonic</b> = Baryonic mass [M<span class="math">\(_\odot\)</span>]</li>
            <li><b>M_total</b> = Total mass (from observations) [M<span class="math">\(_\odot\)</span>]</li>
    ...
    """).props('id="legend-desc"')
        ...
        <ul>
            <li><b>1 kpc</b> = <span class="math">\( 3.086 \times 10^{16} \)</span> m = <span class="math">\( 3.26 \times 10^3 \)</span> light-years</li>
            <li><b>1 pc</b> = <span class="math">\( 3.086 \times 10^{13} \)</span> km = 3.26 light-years</li>
            <li><b>1 Mpc</b> = <span class="math">\( 10^6 \)</span> pc = <span class="math">\( 3.086 \times 10^{19} \)</span> km</li>
            <li><b>1 km/s</b> = <span class="math">\( 3.6 \times 10^3 \)</span> km/h = <span class="math">\( 10^3 \)</span> m/s</li>
            <li><b>1 M<span class="math">\(_\odot\)</span></b> = <span class="math">\( 1.989 \times 10^{30} \)</span> kg (solar mass)</li>
            <li><b>1 L<span class="math">\(_\odot\)</span></b> = <span class="math">\( 3.828 \times 10^{26} \)</span> W (solar luminosity)</li>
        </ul>
    """).props('id="units-desc"')
    ...
    'role=img tabindex=0 aria-label=Rotational velocity curve of the selected galaxy (from data) compared with the baryonic velocity'
                                        ...
                                        with ui.column().classes('flex-1 items-center'):
                                            with ui.pyplot(figsize=(8, 6)):
                                                ...
                                                plt.xlabel('Radius (kpc)'); plt.ylabel(' Mass (10^9 M☉)'); plt.title(f'{os.path.splitext(selected_file)[0]} Enclosed Mass',fontweight='bold')
    ...
    **Galaxy information references:**  
    ...
    """).props('role=dialog aria-modal=true aria-label="Mathematical explanation of virial theorem"')
        ...
        <ul>
            ...
            <li><b>sigma_v</b> = Velocity dispersion of cluster galaxies [km/s]</li>
            <li><b>N</b> = Number of galaxies in the cluster</li>
            <li><b>D</b> = Distance to the cluster [Mpc] (fixed or from redshift <span class="math">\( D = z \cdot c / H_0 \)</span>)</li>
            <li><b>z</b> = redshift</li>
            <li><b>r</b> = Cluster radius [kpc] (computed from angular separation in dataset and distance)</li>
            <li><b>M_total(r)</b> = Total mass (Virial theorem) [M<span class="math">\(_\odot\)</span>]</li>
            <li><b>M_lum(r)</b> = Luminous mass (B-band luminosity × M/L) [M<span class="math">\(_\odot\)</span>]</li>
            <li><b>M/L</b> = 5 (Mass-to-light ratio in B-band) [M<span class="math">\(_\odot\)</span>/L<span class="math">\(_\odot\)</span>]</li>
            <li><b>L(r)</b> = Cumulative B-band luminosity within radius r [L<span class="math">\(_\odot\)</span>]</li>
            <li><b>M_sun_B</b> = 5.48 (absolute magnitude of the Sun in B-band)</li>
            <li><b>H0</b> = 70 km/s/Mpc (Hubble constant)</li>
            <li><b>G</b> = <span class="math">\( 4.302 \times 10^{-6} \)</span> kpc·(km/s)²·M<span class="math">\(_\odot^{-1}\)</span></li>
            <li><b>c</b> = 299792 km/s (Speed of light)</li>
            <li><b>U</b> = Gravitational potential energy of a particle [km²/s²]</li>
            <li><b>F_grav</b> = Gravitational force [M<span class="math">\(_\odot\)</span>·kpc/s²]</li>
            <li><b>F_cent</b> = Centripetal force [M<span class="math">\(_\odot\)</span>·kpc/s²]</li>
            <li><b>v^2(R)</b> = Squared orbital velocity for circular orbit [km²/s²]</li>
            <li><b>K</b> = Kinetic energy of a particle [km²/s²]</li>
            <li><b>2K + U = 0</b> = Virial theorem for bound system</li>
            <li><b>M_circ</b> = Mass corresponding to circular orbit of one particle [M<span class="math">\(_\odot\)</span>]</li>
            <li><b>M_tot(r)</b> = Total virial mass of the cluster [M<span class="math">\(_\odot\)</span>]</li>
            <li><b>L_B</b> = Luminosity of a single galaxy in B-band [L<span class="math">\(_\odot\)</span>]</li>
            <li><b>L(r)</b> = Cumulative luminosity up to radius r [L<span class="math">\(_\odot\)</span>]</li>
            <li><b>M_lum(r)</b> = Luminous mass within radius r [M<span class="math">\(_\odot\)</span>]</li>
            <li><b>D_pc</b> = Cluster distance in parsecs [pc]</li>
            <li><b>dist_mod</b> = Distance modulus</li>
            <li><b>M_abs</b> = Absolute magnitude of a galaxy [mag]</li>
            <li><b>m</b> = Mass of a light single particle or galaxy [M<span class="math">\(_\odot\)</span>]</li>
            <li><b>M</b> = Mass inside radius r of heavy particle or total enclosed mass [M<span class="math">\(_\odot\)</span>]</li>
            <li><b>M_circ</b> = Mass of a particle in circular orbit [M<span class="math">\(_\odot\)</span>]</li>
            <li><b>v^2</b> = Squared orbital velocity [km²/s²]</li>
            <li><b>L_B_i</b> = Luminosity of a single galaxy [L<span class="math">\(_\odot\)</span>]</li>
    ...
    """).props('id="legend-desc"')
        ...
        <h3>Units Conversion</h3>
        <ul>
            <li><b>1 kpc</b> = <span class="math">\( 3.086 \times 10^{16} \)</span> m = 3.26 × 10³ light-years</li>
            <li><b>1 pc</b> = <span class="math">\( 3.086 \times 10^{13} \)</span> km = 3.26 light-years</li>
            <li><b>1 Mpc</b> = <span class="math">\( 10^6 \)</span> pc = <span class="math">\( 3.086 \times 10^{19} \)</span> km</li>
            <li><b>1 km/s</b> = <span class="math">\( 3.6 \times 10^3 \)</span> km/h = <span class="math">\( 10^3 \)</span> m/s</li>
            <li><b>1 M<span class="math">\(_\odot\)</span></b> = <span class="math">\( 1.989 \times 10^{30} \)</span> kg (solar mass)</li>
            <li><b>1 L<span class="math">\(_\odot\)</span></b> = <span class="math">\( 3.828 \times 10^{26} \)</span> W (solar luminosity)</li>
            <li><b>Distance modulus:</b> <span class="math">\( m - M = 5 \log_{10}(d/10\mathrm{pc}) \)</span></li>
            <li><b>Small angle:</b> 1 arcsec at distance D → <span class="math">\( D \cdot \tan(1") \approx D \cdot \frac{1}{206265} \)</span></li>
        </ul>
    """).props('id="units-desc"')
