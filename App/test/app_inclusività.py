...
#from nicegui.elements.canvas import Canvas
...
def speak_text(text: str):
    """
    Inietta il codice JavaScript per la sintesi vocale.
    NiceGUI usa ui.run_javascript per eseguire codice lato client.
    """
    # Evita errori se il testo è vuoto o troppo corto
    if not text or len(text) < 10:
        return

    # Il testo viene pulito per evitare problemi di JS e racchiuso in backticks
    clean_text = text.replace('\n', ' ').replace('"', '\\"').strip()
    
    # Script JavaScript che usa la SpeechSynthesis API del browser
    js_code = f"""
        // Controlla se l'API è supportata
        if ('speechSynthesis' in window) {{
            // Annulla la riproduzione precedente
            speechSynthesis.cancel();
            
            const utterance = new SpeechSynthesisUtterance(`{clean_text}`);
            // Imposta la lingua (es. italiano)
            utterance.lang = 'it-IT';
            // Opzionale: imposta il rate (velocità di lettura)
            utterance.rate = 1.0; 
            
            speechSynthesis.speak(utterance);
        }} else {{
            console.error('TTS non supportato dal browser.');
            ui.notify('Sintesi vocale non supportata dal tuo browser.', type='warning');
        }}
    """
    ui.run_javascript(js_code)
    
def stop_speaking():
    """Annulla la riproduzione TTS in corso."""
    ui.run_javascript("if ('speechSynthesis' in window) { speechSynthesis.cancel(); }")
    
# Variabile per tracciare il carattere attuale (inizialmente vuoto)
current_font_css = ''

def apply_accessibility_settings(font_name: str, high_contrast: bool):
    global current_font_css
    
    # 1. Gestione del carattere (Dislessia)
    if font_name == 'OpenDyslexic':
        # Applica il font Open Dyslexic per il testo
        new_font_css = """
    ...
    # Rimuove il vecchio CSS e aggiunge il nuovo (per la persistenza)
    ui.run_javascript('document.getElementById("custom-font-style")?.remove();')
    ui.add_head_html(new_font_css) 
    
    # ... (resto della funzione per il contrasto) ...
    
    # 2. Gestione del Contrasto
    # NiceGUI usa il tema Quasar. Possiamo forzare il tema scuro per alto contrasto.
    if high_contrast:
        # Tema scuro per alto contrasto
        ui.run_javascript("NiceGUI.app.storage.user.contrast = true; NiceGUI.app.storage.user.high_contrast = true;")
        ui.run_javascript("document.body.classList.add('body--dark')")
    else:
        # Tema chiaro standard
        ui.run_javascript("NiceGUI.app.storage.user.contrast = false; NiceGUI.app.storage.user.high_contrast = false;")
        ui.run_javascript("document.body.classList.remove('body--dark')")

    ui.notify(f'Impostazioni di accessibilità aggiornate: Font={font_name}, Contrasto={high_contrast}')
    # Richiede un ricaricamento della pagina per la modifica CSS completa
    # ui.run_javascript('location.reload()') # Opzionale: ricarica per applicazione totale
...
def accessibility_panel():
    ...
    ui.label('Accessibility settings').classes('text-lg font-bold')
    ui.separator()
        
        # Selezione Carattere
    font_selection = ui.select(
            options=['Standard', 'OpenDyslexic'], 
            label='Carattere', 
            value='Standard'
        )
        
        # Interruttore Alto Contrasto
    contrast_switch = ui.switch(
            text='Alto Contrasto', 
            value=False
        ).props('color=black')

    def update_settings():
        apply_accessibility_settings(font_selection.value, contrast_switch.value)

    font_selection.on_value_change(update_settings)
    contrast_switch.on_value_change(update_settings)
        
        # Placeholder per Daltonismo (richiede modifica dei grafici Plotly)
    ui.label('Suggerimento: i grafici Plotly usano tavolozze accessibili per default.').classes('text-xs italic mt-4')
    ui.button('Close ', 
              on_click=lambda: accessibility_drawer.set_value(False) # <--- CHIUSURA
...
ui.add_head_html("""
    <style>
        /* Aumenta la visibilità del focus per tastiera (tasto TAB) */
        :focus:not(.q-btn--flat):not(.q-btn--outline):not(.q-btn--rounded) {
            outline: 3px solid var(--q-color-negative) !important; /* Bordo rosso brillante */
            outline-offset: 2px !important;
            border-radius: 4px;
        }
    </style>
    ...
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, 'r') as f:
            try:
                ...
                app_data = {'users': {'admin': 'admin'}, 'reflection_log': []}
    # Ensure essential keys exist even if file is new or corrupted
...
async def ask_ai(question: str, response_container: ui.column):
    """Usa Groq API per rispondere alle domande (gratis e veloce)."""
...
async def get_ai_rewrite(text_to_process: str, mode: str):
    """Chiama l'API Groq per semplificare o definire un concetto."""
    
    if mode == "simplify":
        prompt = f"Riscrivi il seguente paragrafo in italiano, usando un linguaggio molto più semplice e diretto, adatto a uno studente di scuola secondaria con DSA. Mantieni i concetti chiave ma elimina il gergo specialistico. Paragrafo: '{text_to_process}'"
    elif mode == "glossary":
        prompt = f"Fornisci una breve definizione in italiano (massimo 2 frasi) del concetto o parola chiave: '{text_to_process}', pensata per studenti di scuola secondaria."
    else:
    ...
    try:
        response = await run.io_bound(
            ...
            model="llama3-8b-8192", # Scegli un modello adatto
        )
...
# Finestra di dialogo per la semplificazione
simplified_dialog = ui.dialog()
with simplified_dialog, ui.card().classes('w-full'):
    ui.label('Easy accessibility').classes('text-xl font-bold')
    simplified_content = ui.markdown('Waiting...').classes('text-lg')
    ui.button('Close', on_click=simplified_dialog.close).props('color=secondary')

async def simplify_text_and_show(text: str):
    """Mostra il dialogo con il testo semplificato."""
    simplified_dialog.open()
    simplified_content.content = 'Waiting...'
    
    # Ottiene la riscrittura
    simplified_text = await get_ai_rewrite(text, "simplify")
    
    # Aggiorna il contenuto del dialogo
    simplified_content.content = simplified_text
...
def main_layout(title: str):
    ...
    ui.page_title(title)
    global accessibility_drawer
    if accessibility_drawer is None:
        # Crea il drawer sul lato destro
        accessibility_drawer = ui.drawer(side='right').props('bordered').classes('p-4 w-1/4') 
        accessibility_panel() # Popola il drawer con i controlli
    ...
    with ui.header(elevated=True).classes('bg-primary text-white items-center justify-between'):
        ...
        with ui.row():
            ...
            ui.button(icon='accessibility', on_click=accessibility_drawer.set_value(True)).tooltip('Accessibility settings').props('flat color=secondary')
...
def standard_module_ui(module_name: str, module_text: str):
    """
    ...
    with ui.card().classes('w-full mt-4'):
        ...
        with ui.row().classes('w-full items-center justify-start gap-4'):
            ...
            # Pulsante per la semplificazione AI
            ui.button('Easy context', icon='auto_fix_high', 
                      on_click=lambda: simplify_text_and_show(module_text)).tooltip("Riscrivi il testo del modulo in modo più semplice").props('color=info outline')

            # Pulsanti per la Sintesi Vocale (TTS)
            ui.button(icon='volume_up', on_click=lambda: speak_text(module_text)).tooltip("Ascolta il testo del modulo").props('color=secondary outline')
            ui.button(icon='stop', on_click=stop_speaking).tooltip("Interrompi lettura").props('color=negative outline')
...
@ui.page('/main')
def main_menu():
    main_layout("Welcome to Cosmo-Edu-Lab")
    global accessibility_drawer
...
# Titolo pensato per card scure (usa la classe .title-on-dark)
def title_on_dark(text: str):
    return ui.label(text).classes("text-4xl font-bold text-center mb-4 title-on-dark")


# Plot info box compatta: tutte le righe in un unico box, stile legenda
def plot_info_box_compact(info: dict, title: str = None, compact: bool = True):
    """
    info: dict {label: value}
    title: opzionale, stringa sopra il box
    compact: se True usa padding/line-height più stretti
    """
    base_classes = "plot-info-box"
    if compact:
        base_classes += " compact"

    # Uso ui.element per poter applicare la classe CSS personalizzata
    with ui.element("div").classes(base_classes):
        ...
        # per ogni riga creiamo struttura flex con label/value
        for lab, val in info.items():
            # crea un contenitore con la classe info-row
...
def get_data_and_images(data_path, img_path, data_extensions=('.txt','.dat', '.csv'), img_extensions=('.jpg', '.png', '.jpeg')):
    """Scans directories for data files and matching image files."""
...
<style>
  ...
  <style>
/* Variante titolo per sfondi scuri (card gradient, ecc.) */
.title-on-dark {
  color: #ffffff !important;
  text-shadow: 0 1px 3px rgba(0,0,0,0.45);
}

/* Plot info box - compatto, righe molto vicine come le legende */
.plot-info-box {
  background: #f8fafc;
...
/* Ogni riga: niente margini, padding ridotto */
...
/* Se vuoi variante ancora più compatta */
...
async function initAudio() {
    ...
    isAudioActive = true;
    console.log("✅ Audio Context attivato con successo dall'utente!");

   
    const btn = document.getElementById('audio-button');
    if (btn) {
        btn.disabled = true;
...
function playSliderTone(val) {
    if (!isAudioActive || !synth) {
        console.warn("⚠️ Audio non attivo o synth mancante");
        return;
    }
    const v = Number(val);
    if (Number.isNaN(v)) {
        console.warn("⚠️ Valore slider non numerico:", val);
        return;
    }
    const freq = 220 + v * 660; // 220Hz → 880Hz
    console.log("🎚️ slider:", v, "→ freq:", freq);
    ...
    with ui.column().classes('w-full p-4 gap-4'):
        ...
        with ui.tab_panels(tabs, value=one).classes('w-full'):
            ...
            with ui.tab_panel(one):
                TEXTG=""""Explore the rotation curve of the NGC3198 galaxy."
                ...
                INFOG=""""**Dataset variables**: Rad (radius), Vobs (observed velocity),  errV (velocity error), Vgas (gas velocity), Vdisk (disk velocity), Vbul (bulge velocity),SBdisk (surface brightness disk),SBbul (surface brightness bulge)"""
                REFG=""""**Dataset reference**: Lelli F. et al., *SPARC: Mass Models for 175 Disk Galaxies with Spitzer Photometry and Accurate Rotation Curves*."""
                ...
                with ui.element('div').classes('text-lg'):
                    ...
                    reference_box(REFG ).classes('text-base italic')
...
''')
                ...
                alpha_slider.on('update:model-value',lambda e: ui.run_javascript(f"window.playSliderTone({float(alpha_slider.value)})"))
                ...
                with ui.column().classes('w-full items-center'):
                    ...
                    with ui.row().classes('w-full justify-center gap-4'):
                        ...
                        chi2_plot_container = ui.column().classes("flex-1 items-center")
    ...
    r_ngc, v_obs_ngc, v_gas_ngc, v_disk_ngc, v_bul_ngc, r_match=r_match)
        ...
        r_ngc, v_obs_ngc, v_gas_ngc, v_disk_ngc, v_bul_ngc, r_match=r_match)
                        ...
                        with mass_plot_container:
                            ...
                            with ui.pyplot(figsize=(8, 5), close=False):
                                ...
                                plt.scatter(r_ngc, m_total_obs, c="b", marker="o", label="Observed mass")
                                plt.plot(r_ngc, m_total_model, "g-", lw=2, label=f"Simulated mass: {np.max(m_total_model):.2e} M☉")
        ...
        "Observed mass": f"{np.max(m_total_obs):.2e} M☉",
        f"Simulated total mass (α={f:.2f})": f"{np.max(m_total_model):.2e} M☉"
    })
                        ...
                        with chi2_plot_container:
                            ...
                            with ui.pyplot(figsize=(8, 5), close=False):
                                if chi2_points:
                                    ...
                                    xs, ys = zip(*chi2_points)
                                    plt.scatter(xs, ys, c="blue", label="Points χ²")
                                    #if len(chi2_points) >= 3:
                                    if len(chi2_points) == 3:
                                        coeffs = np.polyfit(xs, ys, 2) 
                                        x_fit = np.linspace(min(xs), max(xs), 200)
                                        y_fit = np.polyval(coeffs, x_fit)
                                        plt.plot(x_fit, y_fit, "r-", lw=2, label="χ² fit")
                                        y_max_fit = y_fit.max()
                                        y_max_plot = max(y_max_chi, y_max_fit*1.05)
                                    else:
                                        y_max_plot = y_max_chi
                                  
                                    plt.xlim(x_min_chi, x_max_chi)
                                    plt.ylim(y_min_chi, y_max_plot)
                                plt.xlabel("DM mass ($M_\\odot$)")
                                plt.ylabel(r"$\chi^2$/dof")
                                plt.title("χ² vs dark matter mass")
                                plt.grid(True)
                                plt.legend()
                            ui.button("Add χ² point", on_click=add_chi2_point)
                            if chi2_points:
                                last_dm, last_chi2 = chi2_points[-1]
                                plot_info_box_compact({
        "Last point": f"M_DM = {last_dm:.2e} M☉, χ²/dof = {last_chi2:.2f}"
    })
                                if len(chi2_points) >= 3:
                                    coeffs = np.polyfit(xs, ys, 2)
                                    a, b, c = coeffs
                                    if a > 0:  
                                        ...
                                        plot_info_box_compact({
                "Estimated minimum": f"M_DM ≈ {x_vertex:.2e} M☉, χ²/dof ≈ {y_vertex:.2f}"
            })
    ...
    "DM Mass": f"{M_dm_tot:.2e} M☉"
                    ...
                    def on_slider_change():
                        update_galaxy_rotation_plot.refresh()
                        plot_chi2_user_curve.refresh()
                        update_morphology_plot.refresh()
                        update_mass_plot.refresh()

                    alpha_slider.on('update:model-value', on_slider_change)
                    #alpha_slider.on('update:model-value', update_galaxy_rotation_plot.refresh)
                    #alpha_slider.on('update:model-value', lambda: update_galaxy_rotation_plot())
                    #alpha_slider.on('update:model-value', lambda: [update_galaxy_rotation_plot.refresh(), plot_chi2_curve.refresh()])
                    update_galaxy_rotation_plot()
                    update_morphology_plot()
                    plot_chi2_user_curve()
                    
                    update_mass_plot()
                    #ui.button("Plot Chi² vs f", on_click=plot_chi2_curve.refresh).classes("bg-purple-500 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded mt-2")
...
<li>X-axis: velocity, Y-axis: number of galaxies</li>
        ...
        "bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded")      
                ...
                #x_min, x_max = np.min(observed_vel), np.max(observed_vel)
                #padding = (x_max - x_min) * 0.10 if (x_max - x_min) > 0 else 100.0
                #bins = np.linspace(x_min - padding, x_max + padding, 40)
                ...
                #sigma_max = np.sqrt(np.maximum(0.0, G_grav * (m_bary_at_gal + Mc_nfw_enclosed(r_proj_kpc, M200, c, rho_crit)) / (3* r_proj_kpc)))
                #x_min = min(0.0, observed_vel.min())
                #x_max = max(observed_vel.max(), sigma_max.max()) * 1.1
                #padding = (x_max - x_min) * 0.10 if (x_max - x_min) > 0 else 100.0
                #bins = np.linspace(x_min - padding, x_max + padding, 40)
                ...
                #v_all = np.concatenate([observed_vel, simulated_vel])
                #v_min, v_max = v_all.min(), v_all.max()
                #v_min, v_max = observed_vel.min(), observed_vel.max()
                                
                #v_padding = 0.1 * (v_max - v_min)
                ...
                with ui.column().classes('w-full items-center '):
                    ...
                    dm_slider.on('update:model-value', lambda e: ui.run_javascript(f"window.playSliderTone({float(dm_slider.value)})"))
                    with ui.row().classes('w-full justify-center gap-4'):
                        with ui.column().classes('flex-1 items-center'):
                        ...
                        def update_coma_histogram_v2():
                            try:
                                ...
                                #mu_fit, sigma_fit = norm.fit(observed_vel)
                                
                                #M_tot = np.sum(M_total_at_gal)
                                M_bar_tot = np.sum(m_bary_at_gal)
                                ...
                                v_center_f = v_center_bar + (v_center_tot - v_center_bar)
                                #v_mean = np.sqrt(G_grav * M_tot / R_cluster) 
                                #v_mean_bar = np.sqrt(G_grav * M_bar_tot / R_cluster)
                                #v_center = c_light * z_cluster
                                
                                #v_obs_center = np.mean(observed_vel)
                                ...
                                simulated_vel = rng.normal(loc=v_center_f, scale=sigma_fit, size=len(r_proj_kpc))
                                #if f == 0.0:
                                    #sigma_eff = sigma_bar  
                                    #simulated_vel =rng.normal(loc=v_mean_bar, scale=sigma_bar,size=len(r_proj_kpc))
                                  
                               
                                #else:
                                    
                                    #sigma_eff=sigma_los_loc
                                    #simulated_vel =  rng.normal(loc=v_mean, scale=sigma_los_loc,size=len(r_proj_kpc))
                                
                                #simulated_vel = rng.normal(loc=v_obs_center, scale=sigma_eff, size=len(r_proj_kpc))
                                #v_bar = rng.normal(v_mean_bar, sigma_bar, size=len(r_proj_kpc))
                                #v_dm = rng.normal(v_mean, sigma_los_loc, size=len(r_proj_kpc))
                                #v_bar = rng.normal(loc=v_obs_center, scale=sigma_bar, size=len(r_proj_kpc))
                                #v_dm  = rng.normal(loc=v_obs_center, scale=sigma_los_loc, size=len(r_proj_kpc))
                                ...
                                #simulated_vel -= np.mean(simulated_vel)
                                #v_bar -= np.mean(v_bar)
                                #v_dm -= np.mean(v_dm)
                              
                                counts_model, _ = np.histogram(simulated_vel, bins=bins)
                                ...
                                v_model_mean = np.mean(simulated_vel)
                                v_model_sigma = np.std(simulated_vel)
                                ...
                                with plot_container_histo:
                                    ...
                                    with ui.pyplot(figsize=(8, 5)):
                                        plt.hist(observed_vel, bins=bins, alpha=1.0,color='blue', label='Observed Velocities')
                                        #plt.bar(bin_centers, counts_obs, width=np.diff(bins),alpha=1.0, color='blue', label='Observed Velocities',align='center')
                                #plt.bar(bin_centers, counts_model, width=np.diff(bins),alpha=0.7, color='green', label='Simulated Velocities ',align='center')
                                        if len(simulated_vel) > 0:
                                            weights_sim = np.full(len(simulated_vel), len(observed_vel) / len(simulated_vel))
                                            plt.hist(simulated_vel, bins=bins, alpha=0.7,weights=weights_sim, color='green', label=f"Simulated Velocities (v_mean={v_model_mean:.1f} km/s)")
                                        if len(v_bar) > 0:
                                            weights_bar = np.full(len(v_bar), len(observed_vel) / len(v_bar))
                                            plt.hist(v_bar, bins=bins, alpha=0.6, weights=weights_bar,color='red', label='Baryonic Component')
    ...
    "N simulated galaxies (green)": f"{len(simulated_vel)}",
    ...
    "M200, R200, c": f"{M200:.2e} Msun, {R200:.1f} kpc, c={c:.2f}"
                                ...
                                with plot_container_scatter:
                                    ...
                                    with ui.pyplot(figsize=(8, 6)):
                                        ...
                                        plt.scatter(r_proj_kpc, observed_vel, s=10, color='blue', alpha=0.6, label='Observed Galaxies')

                                        plt.scatter(r_proj_kpc, v_bar, s=10, color='red', alpha=0.6, label='Baryonic Component')
                                        plt.scatter(r_proj_kpc, v_dm, s=10, color='green', alpha=0.6, label=f"Simulated Galaxies (DM_mean = {M_dm_mean:.2e} M☉)")
    ...
    "v_max_bar, v_min_bar": f"{v_bar.max():.1f} km/s, {v_bar.min():.1f} km/s"
                        ...
                        #dm_slider.on('update:model-value', lambda _: update_coma_histogram_v2.refresh())
    ...
    "DM factor": f"{f:.2f}"
                    ...
                    def on_slider_change():
                        update_coma_histogram_v2.refresh()
                        update_cluster_points.refresh()
                      

                    dm_slider.on('update:model-value', on_slider_change)
                    #dm_slider.on("update:model-value", lambda _: update_cluster_points.refresh())
                    update_coma_histogram_v2()
                    update_cluster_points()
                    ui.timer(dt, lambda: update_cluster_points.refresh())
...
""").classes('w-full text-center text-base italic mt-2')
                        ...
                        with ui.column().classes('flex-1 items-center'):
                            table_filepath = os.path.join(CLUSTER_TABLES_PATH, "coma_data.csv")
...
"\n\n Click **Run Analysis** to compare **luminous mass** vs **total (virial) mass**.")
                with ui.element('div').classes('text-lg'):
                    info_box("**Dataset variables**: Cluster (name), ID (galaxy ID), RAdeg (right ascension in degrees), DEdeg (declination in degrees), RV (radial velocity in km/s), e_RV (velocity error in km/s), q_RV (quality flag), bmag (apparent magnitude in B band).")
...
""").classes('w-full text-center text-base italic mt-2')
                    ...
                    def check_and_run_cluster():
                        ...
                        ok_s      = s      in {"sigma_v","sigmav","σv","σ_v"}
                        ok_r      = r      in {"r","radius","rad"}
                        ok_g      = g      in {"g","gravitational_constant"}

    
                        ok_v2     = v2     in {"g*m/r","g*m*m/r","gm/r"}

                        ok_K      = K      in {"0.5*m*v^2","0.5*m*v**2","1/2*m*v^2","1/2*m*v**2",
                           "g*m/(2*r)","g*m*m/(2*r)"} 

                        ok_U      = U      in {"-g*m/r","-g*m*m/r"}
                        ok_Fg     = Fg     in {"g*m/r^2","g*m*m/r^2"}     
                        ok_Fc     = Fc     in {"m*v^2/r","m*v**2/r"}      

   
                        ok_virLHS = virLHS in {"k"}
                        ok_virRHS = virRHS in {"u"}

                        ok_Mcirc  = Mcirc  in {"v^2*r/g","v**2*r/g","(v**2)*r/g","r*v^2/g"}
                        ok_Mvir   = Mvir   in {"(v^2)*r/g","(rv^2)*r/g","(rv**2)*r/g",
                           "(rv^2)r/g","(rv**2)r/g"}

                        ok_LB     = LB     in {"m_abs","mabs", "mag_abs"}
                        ok_Lsum   = Lsum   in {"l_b","lb","l_b(r)","lb(r)"}      
                        ok_Mlum   = Mlum   in {"(l(r))", "l(r)"}

                        ok_D      = D      in {"d*1e6","dist*1e6","1e6*d"}     
                        ok_Dmod   = Dmod   in {"d_pc", "dpc", "dist_pc"}
                        ok_Mabs   = Mabs   in {"bmag"}
                        ok_vel    = vel    in {"rv", "rv_i", "rv(i)","v_obs"}

    


                        
                        answer_sigma['el'].classes('w-48')
                        answer_r['el'].classes('w-48')
                        answer_g2['el'].classes('w-48')
                        ans_v2['el'].classes('w-48')
                        ans_K['el'].classes('w-48')
                        ans_U['el'].classes('w-48')
                        ans_Fg['el'].classes('w-48')
                        ans_Fc['el'].classes('w-48')
                        ans_virialLHS['el'].classes('w-48')
                        ans_virialRHS['el'].classes('w-48')
                        ans_Mcirc['el'].classes('w-48')
                        ans_Mvir['el'].classes('w-48')  
                        answer_LB['el'].classes('w-48')
                        answer_Lsum['el'].classes('w-48')
                        answer_Mlum['el'].classes('w-48')
                        answer_D['el'].classes('w-48')
                        answer_Dmod['el'].classes('w-48')
                        answer_Mabs['el'].classes('w-48')
                        answer_vel['el'].classes('w-48')
                        
                        if all([ok_s, ok_r, ok_g, ok_v2, ok_K, ok_U, ok_Fg, ok_Fc, ok_virLHS, ok_virRHS,
        ok_Mcirc, ok_Mvir, ok_LB, ok_Lsum, ok_Mlum, ok_D, ok_Dmod, ok_Mabs, ok_vel]):
                            update_cluster_mass_profile()
                            ui.notify('Correct! Analysis running...', color='positive')
                        else:
                            ui.notify('Wrong! Try again!', color='negative')
                        if not ok_s:
                            answer_sigma['el'].classes('w-48 border-2 border-red-500')
                        if not ok_r:
                            answer_r['el'].classes('w-48 border-2 border-red-500')
                        if not ok_g:
                             answer_g2['el'].classes('w-48 border-2 border-red-500')
                        if not ok_v2:
                             ans_v2['el'].classes('w-48 border-2 border-red-500')   
                        if not ok_K:
                             ans_K['el'].classes('w-48 border-2 border-red-500')
                        if not ok_U:
                             ans_U['el'].classes('w-48 border-2 border-red-500')
                        if not ok_Fg:
                             ans_Fg['el'].classes('w-48 border-2 border-red-500')
                        if not ok_Fc:
                             ans_Fc['el'].classes('w-48 border-2 border-red-500')
                        if not ok_virLHS:
                             ans_virialLHS['el'].classes('w-48 border-2 border-red-500')
                        if not ok_virRHS:
                             ans_virialRHS['el'].classes('w-48 border-2 border-red-500')
                        if not ok_Mcirc:
                             ans_Mcirc['el'].classes('w-48 border-2 border-red-500')
                        if not ok_Mvir:
                             ans_Mvir['el'].classes('w-48 border-2 border-red-500')
                        if not ok_LB:
                             answer_LB['el'].classes('w-48 border-2 border-red-500')
                             
                        if not ok_Lsum:
                             answer_Lsum['el'].classes('w-48 border-2 border-red-500')
                        if not ok_Mlum:
                             answer_Mlum['el'].classes('w-48 border-2 border-red-500')
                        if not ok_D:
                             answer_D['el'].classes('w-48 border-2 border-red-500')
                        if not ok_Dmod: 
                             answer_Dmod['el'].classes('w-48 border-2 border-red-500')
                        if not ok_Mabs:
                             answer_Mabs['el'].classes('w-48 border-2 border-red-500')
                        if not ok_vel:
                             answer_vel['el'].classes('w-48 border-2 border-red-500')
...
""")
                with ui.grid(columns=2).classes('w-full'):
                    ...
                    ui.image('/workspaces/Cosmo-Edu_Lab/App/images/univ_composition.jpg').classes('w-full rounded-lg shadow-lg border border-gray-300')
                    ui.image('/workspaces/Cosmo-Edu_Lab/App/images/power_spectrum.jpg').classes('w-full rounded-lg shadow-lg border border-gray-300')
                    ui.image('/workspaces/Cosmo-Edu_Lab/App/images/CMB_asymmetry.jpg').classes('w-full rounded-lg shadow-lg border border-gray-300')
                    ui.image('/workspaces/Cosmo-Edu_Lab/App/images/Planck_freq.jpg').classes('w-full rounded-lg shadow-lg border border-gray-300')
        
        
        
            FULL_MODULE_TEXT = TEXTG + INFOG + REFG
        standard_module_ui("The Dark Matter and Energy", FULL_MODULE_TEXT)
