...
import asyncio           
# cache per risultati di calcoli pesanti (chiavi: nome dataset o filepath)
_rho_rs_cache = {}
_cluster_calc_cache = {}
...
@app.on_startup
async def increase_js_timeout():
    await ui.run_javascript('window.__nicegui__.requestTimeout = 5000')
...
''')
    ...
    ui.add_head_html('''
<script>
function bindSliderTone(id) {
    const slider = document.querySelector(`#${id} input`);
    if (!slider) return;
    slider.addEventListener('input', (e) => {
        if (typeof playSliderTone === "function") {
            playSliderTone(parseFloat(e.target.value));
        }
    });
}
</script>
...
''')
                ...
                ui.run_javascript(f"bindSliderTone('{alpha_slider.id}')")
                    ...
                    def update_galaxy_mass_analysis():
                        ...
                        with plots_and_image_container:
                            try:
                                ...
                                with ui.row().classes('w-full justify-center gap-4'):
                                    with ui.column().classes('flex-1 items-center'):
                                        with ui.pyplot(figsize=(8, 6),close=False):
                    ...
                    def update_cluster_points():
                        try:
                            ...
                            with sim_container_obs:
                                ...
                                with ui.pyplot(figsize=(4, 4),close=False):
