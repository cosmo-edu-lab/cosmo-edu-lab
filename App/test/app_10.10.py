    ...
    "DM Mass": f"{M_dm_tot:.2e} M☉"
                    ...
                    def perform_parabolic_step():
                        if len(parabolic_state["points"]) != 3:
                            accessible_notify("Initialize first!", type_='warning')
                            return
                        ...
                        if abs(den)<1e-15:
                            accessible_notify("Collinear points are not valid!", type_='warning')
                            return

                        x_new = x2 - 0.5*num/den
                        y_new = chi2_function_for_minimization(x_new)
                        if not np.isfinite(y_new):
                            accessible_notify(f"Value χ² not valid for x={x_new:.2e}.", type_='warning')
                            return
                        ...
                        accessible_notify("Brent minimization step performed.", type_='success')
                        update_displays.refresh()
                        update_all_plots.refresh()
                        plot_chi2_user_curve()
    ...
    'role=status aria-live=polite tabindex=0 aria-label=Plot showing chi-square minimization curve; X axis is dark matter mass in solar masses, Y axis is chi-square per degree of freedom'
                    ...
                    def refresh_formula_from_inputs():
                        ...
                        if None not in [a,b,c,fa,fb,fc]:
                            ...
                            if formula_pts.get("auto_update", False):
                                compute_minimum_formula()
                            else:
                                ...
                                with chi2_plot_container:
                                    ...
                                    with ui.pyplot(figsize=(8,5), close=False):
                                        ax = plt.gca()
                                        ax.scatter(*zip(*points), s=100, c="blue", marker='o', label="Points")
                                        ax.set_xlabel("DM mass ($M_\\odot$)")
                                        ax.set_ylabel(r"χ²/dof")
                                        ax.grid(True)
                                        ax.legend()
