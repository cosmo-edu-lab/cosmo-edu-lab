...
""")
    ...
    with ui.tabs().classes('w-full justify-center') as tabs:
        one = ui.tab('CMB Adiabatic Index', icon='science').props('aria-label="Activity 1: Adiabatic Index"')
        two = ui.tab('Radiation & Matter', icon='public').props('aria-label="Activity 2: Radiation & Matter"')
        three = ui.tab('Photons decoupling', icon='query_stats').props('aria-label="Activity 3: Photons decoupling"')
        four = ui.tab('Entropy', icon='insights').props('aria-label="Activity 4: Entropy"')
    ...
    with ui.tab_panels(tabs, value=one).classes('w-full p-4'):
        ...
        with ui.tab_panel(one).props('role=tabpanel aria-label="CMB Adiabatic Index "'):
            cmb_container = ui.column().classes('w-full p-4')
            add_cmb_activity(cmb_container)
            
        with ui.tab_panel(two).props('role=tabpanel aria-label="Radiation & Matter"'):
            ui.label("Activity 2: Radiation & Matter").classes('text-lg text-gray-600')

    
        with ui.tab_panel(three).props('role=tabpanel aria-label="Photons decoupling"'):
            ui.label("Activity 3: Photons decoupling").classes('text-lg text-gray-600')

      
        with ui.tab_panel(four).props('role=tabpanel aria-label="Entropy"'):
