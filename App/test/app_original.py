...
def title_on_dark(title: str):
    card = ui.card().classes(
        "w-fit mx-auto px-8 py-4 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg shadow-lg mb-4"
    )
    with card:
        el= ui.element('h2').classes("text-4xl font-bold text-center mb-4 title title-on-dark").props(
...
""")
    ...
    def create_interactive_tile(image_file: str, title: str, description: str, size: int):
        ...
        with ui.dialog() as popup, ui.card():
            ...
            aria_button('Close','Close image popup', on_click=lambda: popup.close()).classes('mt-2')
    ...
    def introduction_page(container):
        with container:
            ...
            with ui.element('div').classes('w-full flex justify-center'):
                ...
                on_click=safe_click(lambda: ui.run_javascript("window.open('https://www.solarsystemscope.com/', '_blank')"))).props('outline color=primary')
                aria_button('Explore Universe Scales',"Explore Universe Scales",
                on_click=safe_click(lambda: ui.run_javascript("window.open('https://scaleofuniverse.com/en', '_blank')"))).props('outline color=primary')
                aria_button('Explore the Universe',"Explore the Universe",
                on_click=safe_click(lambda: ui.run_javascript("window.open('https://theskylive.com/3dsolarsystem', '_blank')"))).props('outline color=primary')
                aria_button('Explore the Stars',"Explore the Stars",
                on_click=safe_click(lambda: ui.run_javascript("window.open('https://stellarium-web.org/', '_blank')"))).props('outline color=primary')
                aria_button('NASA simulators',"NASA simulators",
                on_click=safe_click(lambda: ui.run_javascript("window.open('https://science.nasa.gov/eyes/', '_blank')"))).props('outline color=primary')
            ...
            on_click=open_slides_cosmo).props('target=_blank')
                aria_button(
            'Open Cosmo-Edu Lab Activities', 'Open Cosmo-Edu Lab Activities',
            on_click=open_activities).props('target=_blank')

            info_box("**MACRO-COSMOS**: Structures of the Universe").classes("text-2xl m-2").props('role=heading aria-level=2 tabindex=0')
            ...
            with ui.row().classes("w-full gap-6 mt-4"):
                ...
                with ui.column().classes("flex-[1] gap-4"):
                    for label, min_s, max_s in [
                        ...
                        with ui.card().classes('p-3 h-40 w-full relative overflow-auto') as box:
            ...
            aria_button("Reset", "reset", on_click=reset_drop_boxes).classes("bg-red-500 text-white")
...
{"id":"nu_tau","label":"Tau neutrino (ν_τ)","mass":"<0.8 eV","charge":"0","spin":"1/2","desc":"Associated with tau decays","color":"#f6f2ff","shape":"circle"},
    ...
    def create_mass_conversion_dialog():
        ...
        with ui.dialog() as mass_conversion_dialog, ui.card().classes('p-4 w-full max-w-[800px] overflow-x-auto'):
            ...
            ui.html(r"""
                ...
                """, sanitize=False).props('tabindex=0 role=document aria-label="Mass and Energy Conversion information"')
            
        
            aria_button("Close", 'close',on_click=lambda: mass_conversion_dialog.close()).classes("mt-4 float-right")
    ...
    def plot_particle_graph(title, nodes, edges,is_process_graph=False, height=600, width=600 , seed=42):
        ...
        ui.plotly(fig).style(f"height:{height}px; width:{plot_width}")
    ...
    def plot_mass_strip(container, height=280, width=1200, title="Particle masses"):
...
ticktext = [_superscript(i) for i in range(-12, 13)],
    ...
    def particle_page(container):
        with container:
            ...
            info_box("**MICRO-COSMOS**: Particles & Processes").classes(' w-full text-2xl m-4').props('role=heading aria-level=2 tabindex=0')
            with ui.row().classes("w-full flex").style("gap:16px"):
            ...
            with ui.row().classes("w-full flex").style("gap:16px"):
                ...
                plot_particle_graph(
                        "Cosmological Processes (Reactions)",    processes_nodes,  processes_edges,is_process_graph=True,            height=800 , width=1200                     )
            with ui.row().classes("w-full flex").style("gap:16px; margin-top:32px"):
                plot_mass_strip(container, height=320, width=1400)
    ...
    {'year': -280, 'title': 'Aristarchus Proposes Heliocentrism',
     ...
     'desc': 'Aristarchus of Samos proposes a Sun-centered model of the Solar System. [Astronomia](https://www.astronomia.com/progetti/locchio-infinito-di-galileo/prima-di-galileo/aristarco-di-samo-e-la-teoria-eliocentrica/)'},

    {'year': 150, 'title': 'Ptolemy’s Almagest',
     'image': 'ptolemy_almagest.jpg',
     'desc': 'Ptolemy publishes the Almagest, defining 48 constellations and a detailed geocentric model. [OldMaa](https://old.maa.org/press/periodicals/convergence/mathematical-treasure-erasmuss-greek-edition-of-ptolemys-almagest)'},

    # --- Renaissance ---
    {'year': 1543, 'title': 'Copernicus – Heliocentric Model',
     'image': 'copernicus.jpg',
     'desc': 'Copernicus publishes *De Revolutionibus*, reviving the heliocentric theory. [Wikipedia](https://it.wikipedia.org/wiki/De_revolutionibus_orbium_coelestium)'},

    {'year': 1609, 'title': 'Galileo’s Telescope',
     'image': 'galileo_telescope.jpg',
     'desc': 'Galileo builds a telescope and observes Moon craters, Jupiter’s moons, and star fields. [Britannica](https://www.britannica.com/science/Galilean-telescope)'},
    ...
    {'year': 1619, 'title': 'Kepler’s Third Law',
     ...
     'desc': 'Kepler publishes *Harmonices Mundi*, including the third law of planetary motion. [ResearchGate](https://www.researchgate.net/publication/346510651_The_Timeline_Of_Gravity/figures?lo=1&utm_source=google&utm_medium=organic)'},
    ...
    {'year': 1781, 'title': 'Discovery of Uranus',
     ...
     'desc': 'William Herschel discovers Uranus, expanding the Solar System for the first time in history. [BBC](https://www.skyatnightmagazine.com/space-science/how-was-uranus-discovered)'},

    {'year': 1838, 'title': 'First Stellar Parallax',
     'image': 'stellar_parallax.jpg',
     'desc': 'Friedrich Bessel measures the distance to 61 Cygni using parallax — the first direct stellar distance. [Mlahanas](http://www.mlahanas.de/Greeks/Astronomy2.htm)'},

    {'year': 1868, 'title': 'Helium Discovered in the Sun',
     'image': 'helium_sun.jpg',
     'desc': 'Helium is identified in the solar spectrum before its discovery on Earth. [ArmaghPlanet](https://armaghplanet.com/spectroscopy-and-the-discovery-of-helium.html)'},
    ...
    {'year': 1916, 'title': 'Schwarzschild Black Hole Solution',
     ...
     'desc': 'Karl Schwarzschild derives the first exact black hole solution to Einstein’s equations. [DarkCosmos](https://darkcosmos.com/home/f/the-metric-of-a-black-hole)'},

    {'year': 1923, 'title': 'Hubble Discovers Galaxies',
     'image': 'hubble_andromeda.jpg',
     'desc': 'Edwin Hubble proves that Andromeda is an external galaxy beyond the Milky Way. [Astronomy](https://www.astronomy.com/science/how-edwin-hubble-won-the-great-debate/)'},

    {'year': 1929, 'title': 'Cosmic Expansion',
     'image': 'hubble_expansion.jpg',
     'desc': 'Hubble discovers the expansion of the Universe — the Hubble–Lemaître Law. [AstronomyNow-NASA](https://astronomynow.com/2018/10/29/iau-recommends-renaming-hubbles-law-to-include-lemaitre/)'},

    {'year': 1933, 'title': 'Zwicky Proposes Dark Matter',
     'image': 'zwicky_dark_matter.jpg',
     'desc': 'Fritz Zwicky finds missing mass in galaxy clusters → first dark matter evidence. [Nature](https://www.nature.com/articles/d41586-019-02603-7)'},

    # --- Space Age ---
    {'year': 1957, 'title': 'Sputnik 1',
     'image': 'sputnik.jpg',
     'desc': 'The first artificial satellite launches, marking the beginning of the space age. [Wikipedia](https://it.wikipedia.org/wiki/Sputnik_1)'},

    {'year': 1965, 'title': 'Discovery of the CMB',
     'image': 'cmb_penzias_wilson.jpg',
     'desc': 'Penzias and Wilson detect the cosmic microwave background, confirming the Big Bang. [ResearchGate](researchgate.net/publication/271140622_Physics_of_the_cosmic_microwave_background_anisotropy/figures?lo=1&utm_source=google&utm_medium=organic)'},

    {'year': 1969, 'title': 'Apollo 11 Moon Landing',
     'image': 'apollo_11.jpg',
     'desc': 'Neil Armstrong and Buzz Aldrin become the first humans to walk on the Moon. [BBC](https://www.bbc.co.uk/newsround/48789792)'},

    {'year': 1974, 'title': 'Discovery of the First Pulsar Binary',
     'image': 'binary_pulsar.png',
     'desc': 'Hulse and Taylor discover the first binary pulsar — indirect evidence for gravitational waves. [NASA](https://asd.gsfc.nasa.gov/blueshift/index.php/2016/03/17/we-knew-that-already/)'},

    {'year': 1977, 'title': 'Voyager Launch',
     'image': 'voyager.jpg',
     'desc': 'Voyager 1 and 2 begin their Grand Tour of the outer planets and interstellar space. [NASA](https://www.nasa.gov/image-article/voyager-1s-mission-outer-planet-begins/)'},
    ...
    {'year': 1992, 'title': 'First Exoplanets Detected',
     ...
     'desc': 'The first confirmed exoplanets are found around a pulsar by Wolszczan & Frail. [ScientificAmerican](https://www.scientificamerican.com/blog/observations/who-really-discovered-the-first-exoplanet/)'},
    ...
    {'year': 2015, 'title': 'Gravitational Waves Detected',
     ...
     'desc': 'LIGO makes the first direct detection of gravitational waves from merging black holes. [LIGO-Caltech](https://www.ligo.caltech.edu/image/ligo20160211a)'},

    {'year': 2019, 'title': 'First Black Hole Image (EHT)',
     'image': 'm87_eht.png',
     'desc': 'Event Horizon Telescope produces the first image of a black hole (M87*). [EHT](https://eventhorizontelescope.org/blog/astronomers-reveal-first-image-black-hole-heart-our-galaxy)'},
     
    {'year': 2021, 'title': 'JWST Launch',
     'image': 'jwst.jpg',
     'desc': 'The James Webb Space Telescope launches to explore the early Universe. [NASA](https://science.nasa.gov/mission/webb/launch/)'},
    ...
    def show_event_dialog(event):
        with ui.dialog() as dialog, ui.card().classes('w-96'):
            ...
            aria_button('Close','close button', on_click=dialog.close)
        dialog.open()
    ...
    def cosmic_timeline(container):
        with container:
            ...
            with ui.row().classes('w-full items-start gap-4'):
                ...
                with ui.card().classes('p-4 bg-white shadow-md border border-gray-300 max-w-xs'):
    ...
    stellar_nodes = [
        ...
        {
            "id": "massive_star",
            "label": "Massive Star (Main Sequence)",
 ...
 font=dict(size=12) 
    ...
    def show_star_dialog(df, i):
        ...
        with ui.dialog() as dialog, ui.card().classes('w-96'):
            ...
            aria_button("Close",'Close dialog', on_click=dialog.close)
        dialog.open()
    ...
    def hr_diagram_page(container, gaia_csv_path: str,sample_size=20000):
        ...
        with container:
            ...
            fig = px.scatter(
                ...
                title="Gaia H–R Diagram (N={total_stars})",
        ...
        width=650
            ...
            with ui.row().classes('gap-2 mt-4'):
                ui.label("Select star :").classes('text-lg').props('tabindex=0 aria-label=Select star instruction')
                
                star_options = [(f"{row['source_id']} — {row['stellar_phase']}", i) for i, row in df.iterrows()]
                
                select_star = ui.select(
                    options=star_options,
                    label="Star Source ID",
                    with_input=True
                ).classes("w-80").props('aria-label=Star selection dropdown')

                def on_select_star(e):
                    val = e.args
                    if isinstance(val, dict):
                        i = val.get('value')
                        if i is not None and i in df.index:
                            show_star_dialog(df, i)

                #select_star.on('update:model-value', on_select_star)
                '''
    ...
    def galaxy_map_page( container):
        with container:
            ...
            def galaxy_map( csv_path: str,sample_size=100):
                ...
                title_on_dark('Galaxy Map: Explore the Universe')
                ...
                reference_box(""" **Dataset reference**: [SDSS DR16](https://www.sdss.org/dr16/)""")
        ...
        'class': True, 'image_url': False 
                ...
                with ui.row().classes("w-full"):
                    aria_image('/images/sdss_gal.png', "Image of the distribution of galaxies from SDSS dataset").classes('w-1/2 h-auto rounded-lg shadow-lg border border-gray-300')
                    aria_image('/images/sdss_gal_z.png', "Image of the distribution of galaxies with redshift from SDSS dataset").classes('w-1/2 h-auto rounded-lg shadow-lg border border-gray-300')
                    #plot=ui.plotly(fig)
                ...
                with ui.row().classes('gap-2'):
                    ...
                    with ui.dialog() as galaxy_class_dialog, ui.card().classes('p-4 w-full max-w-[800px] overflow-x-auto'):
                        ...
                        ui.html(r"""
                            ...
                            """, sanitize=False).props('tabindex=0 role=document aria-label="SDSS Galaxy Classification information"')
                        
                        
                        aria_button("Close", 'close', on_click=lambda: galaxy_class_dialog.close()).classes("mt-4 float-right")


                    return aria_button(
                        "Galaxy Subclasses Explained", 'Galaxy Classification Info',
                        on_click=lambda: galaxy_class_dialog.open()
                    ).classes("bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded self-center")
    ...
    {'file': 'images/lenticular_galaxy.jpg', 'title': 'Lenticular Galaxy', 'description': '...'}
            ...
            def galaxy_morphology_page(gz_path: str, sample_size=50000):
                with ui.row().classes("w-full gap-4"):
                    with ui.column().classes("flex-1"):
                        title_on_dark("Galaxy Morphology Classification from SDSS")
                        info_box(
                        ...
                        reference_box(""" **Dataset reference**: [SDSS](https://cdsarc.cds.unistra.fr/ftp/J/A+A/648/A122/), VizieR Online Data Catalog: SDSS galaxies morphological classification (Vavilova+, 2021)""")
                    with ui.column().classes('flex-1'):
                        galaxy_image_grid(GALAXY_IMAGES)
                        def show_classification_info():
                            with ui.dialog() as dialog, ui.card().classes("w-96 p-4"):
                                ui.label("Galaxy Classification Explanation").classes("text-lg font-bold text-center").props('aria-label=Galaxy classification explanation title')
                                ui.markdown(
                                    """
                        **Galaxy classification is based on the u−r color index and the concentration index (R50/R90), following observational studies from the Sloan Digital Sky Survey (SDSS).**
                        ...
                        • **Irregular:** very blue colors (u−r < 1.8) and low concentration (C < 2.2)
                                ...
                                aria_button("Close","close", on_click=dialog.close).classes("mt-2 w-full")
                            dialog.open()


                        
                        aria_button("Show Classification Info", "Show Classification Info",on_click=show_classification_info).classes("m-2")
    ...
    hover_data={
                ...
                with ui.row().classes("gap-2 "):
                    ...
                    ui.label("Select galaxy by OBJID:").classes("text-lg mt-4").props('tabindex=0 aria-label=Select galaxy by OBJID instruction')

                    options = [
                        (f"{row['OBJID']} — {row['morphology']}", i)
                        for i, row in df.iterrows()
                    ]

                    select_objid = ui.select(
                        options=options,
                        label="Galaxy OBJID",
                        with_input=True,
                    ).classes("w-80").props('aria-label=Galaxy OBJID selection dropdown').props('aria-describedby=select-galaxy-objid-instruction')
                    
                    def on_select_objid(e):
                        if isinstance(e.args, dict):
                            i = e.args.get("value")
                            if i is not None and i in df.index:
                                show_morphology_dialog(df, i)




                    #select_objid.on('update:model-value', on_select_objid)
                    '''
    ...
    standard_module_ui("Introduction")
...
<p style="text-align:center; margin-top:10px;">NIST Atomic Spectra Database</p> """, sanitize=False).props('tabindex=0 role=document aria-label="Redshift and Flux Relations information"')

                    
                    aria_button("Close", label="Close the box", on_click=lambda: redshift_dialog.close())

        
            aria_button(
"Info",
label="Read detailed information about Redshift and Flux Relations",
on_click=lambda: [
    redshift_dialog.open(),
    ...
    "Ca H": 3968.5,
            ...
            with ui.row().classes("items-center gap-4 mt-3"):
                file_select = ui.select(csv_files, label="Choose spectrum CSV").classes("w-80").props(
            ...
            with ui.row().classes("w-full mt-4 gap-6 items-start"):
                ...
                with ui.row().classes("mt-2 items-center gap-3"):
                    ...
                    check_btn = aria_button("Check table results", "check table results")
    ...
    def dL_Mpc_noLambda(z, H0):
        c = 299792.458
        z = np.atleast_1d(z)
        dL = np.zeros_like(z, dtype=float)
        for i, zi in enumerate(z):
            integrand = lambda zp: 1.0 / np.sqrt(0.3*(1+zp)**3 + 0.0)  # solo materia
...
""", sanitize=False)
                    aria_button("Close", label="Close", on_click=lambda: cosmology_dialog.close())
...
""", sanitize=False)
                    aria_button("Close", label="Close", on_click=lambda: lcdm_dialog.close())
            ...
            def plot_all():
                ...
                with mu_col, ui.pyplot():
                    ...
                    if state['show_mag_hubble']:
                        ...
                        # Calcolo chi2 corretto: O = μ, E = μ_model
...
""", sanitize=False).props("id=planck-info-content role=document aria-live=polite")
            ...
            with container:
                ...
                with ui.dialog() as thermo_dialog, ui.card().classes('p-4 w-full overflow-x-auto'):
                    ui.html(r"""
                        ...
                        """, sanitize=False).props('tabindex=0 role=document aria-label="Thermodynamics and Adiabatic Transformations information"')

                    aria_button("Close", label="Close the box", on_click=lambda: thermo_dialog.close())
                ...
                with ui.dialog() as legend_dialog, ui.card().classes('p-4 w-full overflow-x-auto'):
                    ui.html(r"""
                        ...
                        """, sanitize=False).props('tabindex=0 role=document aria-label="Legend of variables and constants used in the exercise"')

                    aria_button("Close", label="Close the box", on_click=lambda: legend_dialog.close())
                with ui.row().classes("w-full items-start gap-8 mt-4"):
                    ...
                    "**Dataset**: Redshift (z), CMB Temperature (K), T/z+1, T_error"
                        ...
                        with ui.row().classes("items-center gap-3"):
                            aria_button(
                            "Info Thermodynamics",
                            label="Read detailed information about thermodynamics and adiabatic transformations",
                            on_click=lambda: [
                                thermo_dialog.open(),
                                ui.run_javascript("MathJax.typesetPromise()")
                            ]
                            ...
                            aria_button(
                            "Legend",
                            label="Open legend of symbols and units",
                            on_click=lambda: [
                                legend_dialog.open(),
                                ui.run_javascript("MathJax.typesetPromise()")
                            ]
        ...
        def add_radiation_matter_activity(container):
            ...
            with container:
                ...
                """, sanitize=False).props('tabindex=0 role=document aria-label="Equivalence between matter and radiation densities information"')

                    aria_button("Close", label="Close the box", on_click=lambda: eq_dialog.close())
                ...
                """, sanitize=False).props('tabindex=0 role=document aria-label="Legend of symbols and constants used in the exercise"')

                    aria_button("Close", label="Close the box", on_click=lambda: eq_legend_dialog.close())
                with ui.row().classes("w-full items-start gap-8 mt-4"):
                    ...
                    with ui.column().classes("flex-1 items-center bg-gray-900 p-4 rounded-xl shadow-lg"):
                    ...
                    with formula_button_slot:
                        ...
                        with ui.row().classes("mt-6 gap-3"):
                            ...
                            label="Read detailed information about matter-radiation equivalence",
                            on_click=lambda: [
                                eq_dialog.open(),
                                ui.run_javascript("MathJax.typesetPromise()")
                            ]
                            ...
                            aria_button(
                            "Legend",
                            label="Open legend of symbols and constants",
                            on_click=lambda: [
...
''')
    ...
    with ui.column().classes('w-full p-4 gap-4'):
        ...
        with ui.tab_panels(tabs, value=zero).classes('w-full'):
            ...
            with ui.tab_panel(zero).props('role=tabpanel'):
                ...
                with ui.element('div').props('role=region tabindex=0 aria-label=Descriptive text about Kepler Laws activity').classes('text-lg'):
                    info_box(
                    "Explore the orbital characteristics of planets in our solar system using Kepler's Laws."
                    ...
                    "\n\n The third plot presents the mass of each planet vs radius."
                   )
    ...
    """**Dataset reference**: [NASA-SSD](https://ssd.jpl.nasa.gov/planets/) ;[Orbital Mechanics](https://orbital-mechanics.space/reference/planetary-parameters) ;Ryan S. Park, William M. Folkner, James G. Williams, and Dale H. Boggs. The JPL Planetary and Lunar Ephemerides DE440 and DE441. The Astronomical Journal, 161(3):105, February 2021.; Brandon Rhodes. Skyfield: high precision research-grade positions for planets and Earth satellites generator. July 2019""").classes('text-base italic')
                ...
                @ui.refreshable
                def plot_for_planet(selected_planet, df=df0):
                    ...
                    mask_positive = (df['SemiMajorAxis(km)'] > 0) & (df['Velocity(km/s)'] > 0)
                    a_all = df.loc[mask_positive, 'SemiMajorAxis(km)'].values
                    v_all = df.loc[mask_positive, 'Velocity(km/s)'].values

                    a_min = a_all.min() * 0.6
                    a_max = a_all.max() * 1.4
                    a_range = np.logspace(np.log10(a_min), np.log10(a_max), 400)
                    v_kep_range = np.sqrt(G * M_sun / a_range)     # v = sqrt(GM / a)

                  
                    T_theory = 2 * np.pi * np.sqrt(a_range**3 / (G * M_sun)) / (3600 * 24)

                  
                    mask_fit = (df['SemiMajorAxis(km)'] > 0) & (df['Mass(kg)'] > 0)
                    log_a = np.log10(df.loc[mask_fit, 'SemiMajorAxis(km)'].values)
                    log_m = np.log10(df.loc[mask_fit, 'Mass(kg)'].values)
                    p, q = np.polyfit(log_a, log_m, 1)   # log M = p log a + q
                    a_fit = np.logspace(np.log10(a_min), np.log10(a_max), 300)
                    m_fit = 10 ** (p * np.log10(a_fit) + q)
                    pred_logm = p * log_a + q
                    ss_res = np.sum((log_m - pred_logm) ** 2)
                    ss_tot = np.sum((log_m - np.mean(log_m)) ** 2)
                    r2 = 1 - ss_res / ss_tot if ss_tot != 0 else np.nan

                    with ui.pyplot(figsize=(18, 5), close=True, clear=True):

                       
                        plt.subplot(1, 3, 1)
                        plt.plot(a_range / 1e6, v_kep_range, '--', color='gray', lw=2,
                        ...
                        for pnm in planets:
                            ...
                            plt.scatter(pa / 1e6, pv, color=planet_colors[pnm], s=70, zorder=5)
                            plt.text(pa / 1e6 * 1.03, pv * 1.03, pnm, fontsize=9, color=planet_colors[pnm])

                        
                        plt.scatter(a_sel / 1e6, v_sel, edgecolors='k', linewidths=1.2, facecolors='none', s=200, zorder=6)
                        plt.xlabel("Semi-major axis (10^6 km)")
                        plt.ylabel("Orbital velocity (km/s)")
                        plt.title("Keplerian Orbital Velocity Curve for Planets ")
                        ...
                        for pnm in planets:
                            ...
                            plt.text(pa / 1e6 * 1.03, pT * 1.03, pnm, fontsize=8, color=planet_colors[pnm])
                        ...
                        for pnm in planets:
                            ...
                            plt.text(pa / 1e6 * 1.03, pm * 1.03, pnm, fontsize=8, color=planet_colors[pnm])

                        #plt.plot(a_fit / 1e6, m_fit, '--', color='black', lw=1.8,    label=f'Fit: M = 10^q · a^p, p={p:.2f}, R²={r2:.2f}')
                        plt.xscale('log'); plt.yscale('log')
                        plt.xlabel("a (10^6 km)")
                        plt.ylabel("Mass (kg)")
                        ...
                        #plt.subplot(2, 3, 4)
                        #plt.axis('off')
                        #plt.text(0.02, 0.98, f"Selected: {selected_planet}", va='top', ha='left', fontsize=12)
                ...
                def info_panel(selected_planet, df=df0):
                    ...
                    html += f"""
                    <div style='flex:1;'>
                        <h2 style='margin:0; color:{title_color};'>{selected_planet}</h2>
                    ...
                    ui.html(html, sanitize=False).props('role=region tabindex=0 aria-label=Planet information panel')

                def open_slides():
                    ui.run_javascript('window.open("/slides/cosmo_dark_matter.pdf", "_blank")')
                with ui.row().style("align-items:flex-start; gap:32px; width:100%;"):
          ...
          on_click=safe_click(lambda: ui.run_javascript("window.open('https://phet.colorado.edu/sims/html/keplers-laws/latest/keplers-laws_all.html', '_blank')"))).props('outline color=primary')
                        

                        aria_button('Open Introduction Presentation', 'Open Introduction Presentation',on_click=open_slides).props('outline color=primary')
                        #ui.html('''<iframe src="/slides/cosmo_dark_matter.pdf#toolbar=1" width="100%" height="800px" style="border:none;"></iframe>''',sanitize=False).props('role=region tabindex=0 aria-label= slides')
                        
                with ui.column().style("width:100%; margin-top:24px;"):
                    plot_for_planet(selector.value)
                        
                        
                selector.on('update:model-value', lambda e: (     plot_for_planet.refresh(selector.value),        info_panel.refresh(selector.value)    ))
...
"""
                ...
                def update_table():
                    ...
                    ui.html("<h5>Galaxy Information</h5>",sanitize=False)
...
$$
    ...
    'role=status aria-live=polite tabindex=0 aria-label=Galaxy morphology representation (from above), showing disk with baryonic component (stars,gas) and halo appears adding dark matter (slider)'
                                ...
                                plot_info_box_compact({
    "Disk and Bulge Mass": f"{M_bary_tot:.2e} M☉",
...
""").classes('w-full text-center text-base italic mt-2')
                ...
                answer_Mtot, answer_Mbar, answer_vgas, answer_vdisk, answer_vbulge]:
                    ...
                    with ui.dialog() as plots_popup, ui.card().classes('p-4 w-full max-w-[1300px]'):
                        ...
                        aria_button("Close", "close the plots popup", on_click=plots_popup.close)

                    with ui.row().classes('w-full justify-center mt-4 mb-8'):
                        with ui.column().classes('flex-1 items-center'):
                            aria_button("Run Analysis", "Run the analysis to reproduce the plots",on_click=check_and_run_galaxy)
                        with ui.column().classes('flex-1 items-center'):
                    
                            aria_button("Open Plots", "Open the two galaxy plots in a popup window",            on_click=lambda: [update_plots_popup(), plots_popup.open()]).classes("bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded ml-2")
...
""", sanitize=False)
                ...
                def update_cluster_table():
                    select = cluster_state["select"]
                    ui.html("<h5>Cluster Information</h5>",sanitize=False)
                    if select.lower() == "coma_data.csv":
...
"M200, R200, c": f"{M200:.2e} Msun, {R200:.1f} kpc, c={c:.2f}"
                        ...
                        with plot_container_scatter:
                            ...
                            #plt.plot(r_sorted, v_lower, 'r--', lw=1.5)
                                ...
                                plt.title("Scatter plot of cluster galaxies velocities")
                                plt.legend()
...
""").classes('w-full text-center text-base italic mt-2')
                    ...
                    with ui.dialog() as cluster_plots_popup, ui.card().classes("p-4 w-full max-w-[1300px]"):
                        ...
                        aria_button("Close", "close popup", on_click=cluster_plots_popup.close)
                    with ui.row().classes('w-full justify-center mt-4'):
                        with ui.column().classes('flex-1 items-center '):
                        ...
                        with ui.column().classes('flex-1 items-center '):
                            ...
                            aria_button("Open Cluster Plots",             "Open cluster mass & density plots",    on_click=lambda: [update_cluster_popup_plots(), cluster_plots_popup.open()]).classes("bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded")
            ...
            with ui.tab_panel(five).props('role=tabpanel'):
                ...
                with ui.row().classes('w-full gap-4 justify-start'):
                    with ui.column().classes('flex-1 max-w-md items-start '):
                        aria_button('Additional Information:Numerical Explanation',"Additional Information:Numerical Explanation", on_click=open_slides_DM).props('outline color=primary')
