...
def aria_button(text: str, label: str, **kwargs):
    ...
    return ui.button(text, **kwargs).props(f'aria-label={label}')
...
def aria_chart_label(description: str):
    ...
    return ui.label(description).props('class=sr-only')


def aria_table(columns, rows, label: str, **kwargs):
    
    return ui.table(columns=columns, rows=rows, **kwargs).props(f'role=table aria-label={label}')


def aria_input(label: str, aria_label: str, **kwargs):
   
    return ui.input(label, **kwargs).props(f'aria-label={aria_label}')

def aria_textarea(label: str, aria_label: str, **kwargs):
   
    return ui.textarea(label, **kwargs).props(f'aria-label={aria_label}')
...
def aria_slider(*, min_value=None, max_value=None, min=None, max=None, value=0.0, step=0.01, aria_label='', **kwargs):
    ...
    return ui.slider(min=min_v, max=max_v, value=value, step=step, **kwargs).props(f'aria-label={aria_label}')


def aria_formula_input(**kwargs):
 
    return ui.input(label="", **kwargs).props('aria-label="Input the missing term in the formula"')
...
def main_layout(title: str):
    ...
    with ui.header(elevated=True).classes('bg-primary text-white items-center justify-between'):
        ...
        with ui.row():
            aria_button('🏠 Home', "Go to the homepage", on_click=lambda: ui.navigate.to('/main'), icon='home')
            aria_button('🔙 Back',"Go back to previous page", on_click=lambda: ui.run_javascript('window.history.back()'), icon='arrow_back')
    ...
    with ui.left_drawer().classes('bg-gray-100 shadow-lg'):
        ...
        aria_button('🏠 Home', "Go to the homepage",on_click=lambda: ui.navigate.to('/main')).classes('w-full')
        aria_button('📚 Cosmology Modules', "Go to the cosmology modules", on_click=lambda: ui.navigate.to('/main')).classes('w-full')
        aria_button('🧭 Physics → Cosmology', "Analyze the connection between classical Physics and Cosmology topics",on_click=lambda: ui.navigate.to('/physics-links')).classes('w-full')
        aria_button('🧪 Physics Curriculum', "Connections between Physics program at secondary school and cosmology modules",on_click=lambda: ui.navigate.to('/physics-program')).classes('w-full')
        aria_button('📝 Reflections', "Read the user's reflections",on_click=lambda: ui.navigate.to('/reflections')).classes('w-full')
...
def standard_module_ui(module_name: str):
    ...
    with ui.card().classes('w-full mt-4'):
        ...
        aria_button('Ask Question',"Ask a question to the AI Tutor",
                    on_click=lambda: ask_ai(q_input.value, ai_response_container)).classes('mt-2')
    ...
    with ui.card().classes('w-full mt-4'):
        ...
        aria_button('Save Reflection', "Save your reflection",
                    on_click=lambda: submit_reflection(module_name, reflection_input.value)).classes('mt-2')
        ...
        with ui.grid(columns=4).classes('gap-4 m-4 w-full'):
            ...
            for i, title in enumerate(module_titles, 1):
                with ui.card().classes('w-full hover:shadow-xl transition'):
                    ...
                    aria_button(f'Explore Module {i}', "Go to module", on_click=lambda i=i: ui.navigate.to(f'/module{i}')).classes('mt-4 w-full')

       
        aria_button('🧭 Physics → Cosmology Connections', "Open classical physics topics", on_click=lambda: ui.navigate.to('/physics-links')).classes('mt-4')
        aria_button('🧪 Physics Curriculum', "Open the physics curriculum", on_click=lambda: ui.navigate.to('/physics-program')).classes('mt-2')
        aria_button('📝 View Reflections',"View the user's reflections" ,on_click=lambda: ui.navigate.to('/reflections')).classes('mt-2')
...
def physics_links():
    ...
    with ui.row():
        for title, classical, cosmo, link in table_data:
            aria_button(f"Go to {title}","Got to cosmology module", on_click=lambda l=link: ui.navigate.to(l)).classes('mr-4')
    ...
    for year, topics in physics_curriculum:
        with ui.expansion(year, icon='school').classes('w-full'):
            for topic, link in topics:
                if link:
                    aria_button(topic, " Go to cosmology topic linked to physics topic",on_click=lambda l=link: ui.navigate.to(l)).classes('w-full text-left')
                else:
...
''')
    ...
    ui.label("Select a section to explore dark matter activities").props('id=tabs_desc')
    ...
    with ui.column().classes('w-full p-4 gap-4'):
        ...
        with ui.tab_panels(tabs, value=one).classes('w-full'):
            with ui.tab_panel(one).props('role=tabpanel'):
                ...
                with ui.element('div').props(' role=region aria-label=Descriptive text about galaxy rotation curve activity').classes('text-lg'):
                    paragraph(
                ...
                with ui.element('div').props(' role=region aria-label=Dataset variables').classes('text-lg'):
...
<li>Y-axis: <span class="math">\( v_{\mathrm{bar}} \)</span> (red), <span class="math">\( v_{\mathrm{obs}} \)</span> (blue with grey error bars), <span class="math">\( v_{\mathrm{tot,sim}} \)</span> (green)</li>
        ...
        """, sanitize=False).props(' aria-label=Mathematical explanation of the computational steps of the galaxy rotation curve activity')
...
$$
                        ...
                        #aria_button("Perform Parabolic Step", on_click=lambda: perform_parabolic_step())
                                aria_button("Compute minimum","Compute the minimum from three input points (dark matter mass and chi-squared values)", on_click=lambda: compute_minimum_formula())
    ...
    'role=status aria-live=polite aria-label=Rotational velocity curve of the galaxy (from data),baryonic velocity curve (computed from data) and simulated velocity curve updated with new dark matter value (slider moved)'
)
        ...
        r_ngc, v_obs_ngc, v_gas_ngc, v_disk_ngc, v_bul_ngc, r_match=r_match)
                        ...
                        with mass_plot_container:
                            ...
                            with ui.pyplot(figsize=(8, 5), close=False):
                                ...
                                plt.grid(True)
                                ui.element('div').props(
    'role=status aria-live=polite aria-label=Total mass of the galaxy (from data), baryonic mass (computed from data) and simulated mass updated with new dark matter value'
)
                    ...
                    def update_morphology_plot():
                            ...
                            with morph_plot_container:
                                ...
                                with ui.pyplot(figsize=(5,5), close=False):
                                    ...
                                    ax.legend()
                                    ui.element('div').props(
    'role=status aria-live=polite aria-label=Galaxy morphology representation (from above), showing disk with baryonic component (stars,gas) and halo appears adding dark matter (slider)'
)
    ...
    "DM Mass": f"{M_dm_tot:.2e} M☉"
                    ...
                    def initialize_parabolic_points():
                        ...
                        ui.notify("Points computed. Check the minimum on the plot.", type='positive')
                            ...
                            with ui.pyplot(figsize=(8,5), close=False):
                                ...
                                if parabolic_state["plot_points"]:
                                    ...
                                    if len(all_points) >= 3:
                                        ...
                                        ax.plot(x_fit_alg, y_fit_alg, "black--", lw=2, label="Algorithm Fit", zorder=8)
                                ...
                                if manual_points and len(manual_points) >= 3:
                                    ...
                                    result_label.set_text(f"x_min ≈ {xmin_manual:.3e}, χ² ≈ {ymin_manual:.4f}")
                                    
                                if auto_points and len(auto_points) >= 3:
                                    xa, ya = zip(*auto_points)
                                    ax.scatter(xa, ya, s=60, c="blue", marker="o", label="Points")

                                    coeffs_auto = np.polyfit(xa, ya, 2)
                                    x_fit_auto = np.linspace(min(xa), max(xa), 400)
                                    y_fit_auto = np.polyval(coeffs_auto, x_fit_auto)

                                    ax.plot(x_fit_auto, y_fit_auto, "green", lw=2, label="Parabolic Fit")

                                  
                                    xmin_auto = -coeffs_auto[1] / (2 * coeffs_auto[0])
                                    ymin_auto = np.polyval(coeffs_auto, xmin_auto)
                                    ax.scatter([xmin_auto], [ymin_auto], c='red', s=120, marker='*', label=" Min")

                                    result_label.set_text(f"x_min ≈ {xmin_auto:.3e}, χ² ≈ {ymin_auto:.4f}")
                                ...
                                ui.element('div').props(
    'role=status aria-live=polite aria-label=Plot showing chi-square minimization curve; X axis is dark matter mass in solar masses, Y axis is chi-square per degree of freedom'
)
                    ...
                    def compute_minimum_formula():
                        ...
                        with chi2_plot_container:
                            ...
                            with ui.pyplot(figsize=(8,5), close=False):
                                ...
                                ax.scatter([a],[fa], s=100, c="blue", marker='o', label="f(a)")
                                ax.scatter([b],[fb], s=100, c="blue", marker='o', label="f(b)")
                                ax.scatter([c],[fc], s=100, c="blue", marker='o', label="f(c)")
                                ax.plot(x_fit, y_fit, 'green', lw=2, label="Parabolic fit")
                                ax.axvline(xmin, color='red', linestyle=':', lw=2, label=f"x_min ≈ {xmin:.3e}")
                                ax.scatter([xmin], [ymin], c='red', s=140, marker='*', zorder=10)
...
""").classes('w-full text-center text-base italic mt-2')
            ...
            with ui.tab_panel(two).props('role=tabpanel'):
                ...
                with ui.element('div').props(' role=region aria-label=Descriptive text about galaxy velocity and mass activities').classes('text-lg'):
                    paragraph(
    ...
    "\n\n Click **Run Analysis** to compare the baryonic mass with the total mass and the observed velocity with the Keplerian-like velocity.")
                with ui.element('div').props(' role=region aria-label=Descriptive text about dataset variables').classes('text-lg'):
                    info_box( "**Dataset variables**: Rad (radius), Vobs (observed velocity),  errV (velocity error), Vgas (gas velocity), Vdisk (disk velocity), Vbul (bulge velocity),SBdisk (surface brightness disk),SBbul (surface brightness bulge)")
...
<li>Also plot masses: <span class="math">\( M_{\mathrm{bar}} \)</span> (red), <span class="math">\( M_{\mathrm{tot}} \)</span> (blue)</li>
        ...
        """, sanitize=False).props(' aria-label=Mathematical explanation of the steps to compute baryonic velocity and mass from data')
                                    ...
                                    with ui.row().classes('w-full justify-center gap-4'):
                                        with ui.column().classes('flex-1 items-center'):
                                            with ui.pyplot(figsize=(8, 6)):
                                                ...
                                                ui.element('div').props(
    'role=img aria-label=Rotational velocity curve of the selected galaxy (from data) compared with the baryonic velocity'
)
                                        ...
                                        with ui.column().classes('flex-1 items-center'):
                                            with ui.pyplot(figsize=(8, 6)):
                                                ...
                                                ui.element('div').props(
    'role=img aria-label=Total mass and baryonic mass of the selected galaxy (from data)'
)
...
""").classes('w-full text-center text-base italic mt-2')
            ...
            with ui.tab_panel(three).props('role=tabpanel'):
                ...
                with ui.element('div').props(' role=region aria-label=Descriptive text about galaxy cluster activity').classes('text-lg'):
                ...
                with ui.element('div').props(' role=region aria-label=Descriptive text about Coma cluster dataset').classes('text-lg '):
...
<li>X-axis: velocity, Y-axis: number of galaxies</li>
        ...
        """, sanitize=False).props(' aria-label=Mathematical explanation of the steps to compute cluster properties from data')
                        aria_button("Close","Close the box", on_click=cluster_dialog.close)
                        
                        
                        
                        
                aria_button("Info", "Read the detailed information about computational steps from data to plots",on_click=lambda: [cluster_dialog.open(), ui.run_javascript("MathJax.typesetPromise()")]).classes(
        "bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded")      
                            ...
                            try:
                                ...
                                with plot_container_histo:
                                    ...
                                    with ui.pyplot(figsize=(8, 5)):
                                        ...
                                        ui.element('div').props(
    'role=status aria-live=polite aria-label=Histogram showing the observed velocities of galaxies in the Coma cluster in blue,the baryonic component in red and the simulated velocities in green, which depend on the dark matter fraction set by the slider'
)
    ...
    "M200, R200, c": f"{M200:.2e} Msun, {R200:.1f} kpc, c={c:.2f}"
                                ...
                                with plot_container_scatter:
                                    ...
                                    #plt.plot(r_sorted, v_lower, 'r--', lw=1.5)
                                        ...
                                        ui.element('div').props(
    'role=status aria-live=polite aria-label=Scatter plot showing the observed velocities of galaxies in the Coma cluster in blue,the baryonic component in red and the simulated velocities in green, which depend on the dark matter fraction set by the slider'
)
                        ...
                        try:
                            ...
                            with sim_container_obs:
                                ...
                                with ui.pyplot(figsize=(4, 4)):
                                    ...
                                    ui.element('div').props(
    'role=status aria-live=polite aria-label=Scatter plot showing the positions and movements of observed galaxies in the Coma cluster '
)
                                plot_info_box_compact({
    ...
    "σ_obs": f"{sigma_obs_val:.0f} km/s"
                            ...
                            with sim_container_model:
                                ...
                                with ui.pyplot(figsize=(4, 4)):
                                    ...
                                    plt.legend(loc="upper right")
                                    ui.element('div').props(
    'role=status aria-live=polite aria-label=Scatter plot showing the positions and movements of simulated galaxies in the Coma cluster which depend on the dark matter fraction set by the slider'
)
                                plot_info_box_compact({
...
""").classes('w-full text-center text-base italic mt-2')
            ...
            with ui.tab_panel(four).props('role=tabpanel'):
                ...
                with ui.element('div').props(' role=region aria-label=Descriptive text about galaxy cluster mass and density activities').classes('text-lg'):
                    paragraph(
...
"\n\n Click **Run Analysis** to compare **luminous mass** vs **total (virial) mass**.")
                with ui.element('div').props(' role=region aria-label=Descriptive text about cluster datasets').classes('text-lg'):
...
""", sanitize=False).props(' aria-label=Mathematical explanation of the formulas used in the cluster mass and virial theorem activity')
                            ...
                            try:
                                ...
                                with ui.column().classes('flex-1 items-center'):  
                                    with ui.pyplot(figsize=(8,6)):
                                        ...
                                        ui.element('div').props(
    'role=img aria-label=Plot showing the mass profile (in solar mass) in function to the radius (in kpc) of the selected galaxy cluster, comparing luminous mass and total mass'
)
                                ...
                                with ui.column().classes('flex-1 items-center'):
                                    with ui.pyplot(figsize=(8,6)):
                                        ...
                                        plt.legend()
                                        ui.element('div').props(
    'role=img aria-label=Plot showing the mass density profile in function to the radius of the selected galaxy cluster, comparing luminous mass and total mass'
)
...
""")
                    with ui.column().classes('flex-1 max-w-md items-start'):
                        aria_button('ESA: The Universe According to Planck', "ESA reference website",on_click=lambda: ui.run_javascript("window.open('https://sci.esa.int/web/planck/-/51551-simple-but-challenging-the-universe-according-to-planck','_blank')")).props('outline color=accent')
                        reference_box("""
