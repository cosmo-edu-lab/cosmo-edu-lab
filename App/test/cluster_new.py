...
def run_cluster_analysis():
    ...
    output_dir = "cluster_plot_new"
    os.makedirs(output_dir, exist_ok=True)
    rho_crit = rho_crit_Msunkpc3()

    print(f"{'Cluster':<12} | {'χ² Min':<8} | {'M_DM (M_sun)':<12} | {'M_tot (M_sun)':<12} | {'M_bar (M_sun)':<12} | {'R200 (kpc)':<10} | {'% DM'}")
    print("-" * 90)
    results_data = []
    ...
    for file in sorted(all_files):
        ...
        try:
            ...
            print(f"{cluster_name:<12} | {chi2_min:<8.2f} | {M_DM:<12.2e} | {M_tot_final:<12.2e} | {M_bar_tot:<12.2e} | {R200:<10.1f} | {perc_DM:.1f}%")
            ...
            v_dm  = rng.normal(loc=v_mean_obs, scale=sigma_global, size=len(r_sorted))
    ...
    export_file = "cluster_best_parameters_virial.csv"
